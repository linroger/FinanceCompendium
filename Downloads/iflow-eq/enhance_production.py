#!/usr/bin/env python3
"""
Financial Equations Enhancement Script - Production Version v2

Order of operations:
1. Fix subscripts/superscripts
2. Add multiplication (before wrapping to avoid issues)
3. Wrap multi-char abbreviations in \text{}
4. Expand basic formulas
...
def fix_subscripts_superscripts(eq):
    ...
    # Handle multi-character subscripts first (like domestic, foreign)
    eq = re.sub(r'_([a-zA-Z]{2,})', r'_{\1}', eq)
    # Then handle single-char subscripts
...
def add_multiplication(eq):
    """Add explicit \\times for implicit multiplication, avoiding multi-char abbreviations"""
    # Multi-char abbreviations that should NOT have \times between letters
    abbrevs = ['FV', 'PV', 'DF', 'CF', 'CD', 'AI', 'EAR', 'YTM', 'YTC', 'YTW', 'CY',
               'VaR', 'CVaR', 'ES', 'IR', 'SR', 'ROE', 'ROA', 'WACC', 'NPV', 'IRR',
               'EBIT', 'EBITDA', 'EVA', 'ROI', 'CAPM', 'APT', 'SML', 'CML', 'BS', 'GBM',
               'MOIC', 'LPME', 'LP', 'GP', 'CoC', 'GRM', 'CRF', 'DSCR', 'LTV',
               'NOI', 'EGI', 'MDD', 'TV', 'PI', 'APV', 'ROIC', 'FCFF', 'FCFE', 'NOPAT']

    # For each position, check if adding \times would split an abbreviation
    def should_add_times(eq, pos):
        # Get surrounding context
        left = eq[:pos]
        right = eq[pos:]

        # Check if we're between letters that form an abbreviation
        # Pattern: letter1 + \times + letter2
        # We only add \times if letter1+letter2 is NOT an abbreviation

        # Find the letter before our position (skipping spaces)
        i = pos - 1
        while i >= 0 and eq[i] == ' ':
            i -= 1
        if i < 0:
        ...
        # Find the letter after our position (skipping spaces)
        j = pos
        ...
        # Check if this pair forms an abbreviation
        potential_abbr = letter1 + letter2
        if potential_abbr in abbrevs:
            return False

        return True

    # Now process the equation
    result = []
    i = 0
    while i < len(eq):
        char = eq[i]

        # Check if we're at a position where we might need \times
        if i > 0 and i < len(eq) - 1:
            prev_char = eq[i-1]
            next_char = eq[i+1]

            # Check for patterns that need multiplication
            # Letter followed by ( or number or letter
            if (prev_char.isalpha() and char == ' ' and
                (next_char == '(' or next_char.isalnum())):
                # Check if this creates an abbreviation
                potential_abbr = prev_char + next_char
                if potential_abbr not in abbrevs:
                    result.append(' \\times ')
                    i += 1
                    continue

            # Number followed by letter (but not in subscript/superscript)
            if (prev_char.isdigit() and char == ' ' and
                next_char.isalpha()):
                result.append(' \\times ')
                i += 1
                continue
...
def wrap_variables_in_text(eq):
    ...
    multi_char_vars = [
        ...
        r'\bP_E\b', r'\bP_0\b', r'\bS_0\b', r'\bK_0\b',
        ...
        r'\bMRP\b', r'\bDPR\b', r'\bGRM\b', r'\bD&A\b',
...
def main():
    ...
    categories_to_process = [
        ...
        ('category8_corporate_finance', 'category8_corporate_finance_enhanced_v3'),
        ...
        ('category9_alternative_investments', 'category9_alternative_investments_enhanced_v3'),
        ('category10_fx_derivatives', 'category10_fx_derivatives_enhanced_v3'),
    ]
