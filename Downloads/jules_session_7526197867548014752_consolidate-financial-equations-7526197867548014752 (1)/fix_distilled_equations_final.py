...
def fix_discount_factor_entries(title, equation, line_num):
    ...
    # Mapping of specific line numbers to correct titles based on the equation content
...
def manual_corrections(records):
    """Apply manual corrections for specific known issues"""
    
    # Manual corrections based on line numbers
    manual_fixes = {
        3: {  # Line 3 - Natural Gas Storage
            'title': 'Natural Gas Storage (Working\'s Model)',
            'equation': 'V = E[ max( S_T - S_0 exp((r+u)T), 0) ]',
            'notes': 'Commodity pricing model'
        },
        112: {  # Line 112 - FX Forward Points
            'title': 'FX Forward Points',
            'equation': 'Points = S_0 * (r_d - r_f) * T',
            'notes': 'Forward premium/discount. Points = F - S. Quoted as pips. Basis from CIP deviation.'
        },
        113: {  # Line 113 - Fokker-Planck
            ...
            'notes': 'Forward PDE for probability density under local vol.'
        },
        114: {  # Line 114 - Geometric Series
            'title': 'Geometric Series',
            'equation': '\\sum_{k=0}^\\infty r^k = \\underbrace{\\frac{1}{1-r}}_{\\text{Sum}} \\quad \\text{for } |r| < 1',
            'notes': 'The geometric series is the mathematical foundation for discounting cash flows.'
        },
        115: {  # Line 115 - Gold Lease Rate
            'title': 'Gold Lease Rate',
            'equation': 'LeaseRate = r_risk-free - Cost of Carry',
            'notes': 'Return on gold lending. Negative lease rate common (gold has cost of storage but no income).'
        },
        117: {  # Line 117 - Key Rate Duration
            'title': 'Key Rate Duration',
            'equation': 'KRD_k = -(1/P) \\partial P / \\partial y_k',
            'notes': 'Sensitivity to specific yield curve points.'
        },
        118: {  # Line 118 - Key Rate Duration Alternative
            'title': 'Key Rate Duration (Alternative)',
            'equation': 'D_{KR}(t) = -\\frac{1}{P} \\frac{\\partial P}{\\partial r_t}',
            'notes': 'Key Rate Duration measures the price sensitivity to a shift in a specific point of the yield curve.'
        },
        119: {  # Line 119 - Kolmogorov Forward Equation
            'title': 'Kolmogorov Forward Equation',
            'equation': '\\frac{\\partial p(x,t)}{\\partial t} = -\\frac{\\partial}{\\partial x}[\\mu(x,t) p(x,t)] + \\frac{1}{2}\\frac{\\partial^2}{\\partial x^2}[\\sigma^2(x,t) p(x,t)]',
            'notes': 'Forward equation for probability density evolution.'
        },
        123: {  # Line 123 - FX Forward Points
            'title': 'Basket Option Moment Matching',
            'equation': '\\sigma^2 = \\log(E[(S_T^Basket)^2] / (E[S_T^Basket])^2)',
            'notes': 'Parameter estimation for basket option pricing.'
        }
    }
    ...
    for i, record in enumerate(records):
        ...
        if line_num in manual_fixes:
            fixes = manual_fixes[line_num]
            if 'title' in fixes:
                records[i][0] = fixes['title']
            if 'equation' in fixes:
                records[i][1] = fixes['equation']
            if 'notes' in fixes:
                records[i][3] = fixes['notes']
...
def process_csv(input_file, output_file):
    ...
    # Apply manual corrections for specific known issues
    records = manual_corrections(records)
...
if __name__ == "__main__":
    ...
    output_file = "/Users/rogerlin/Downloads/jules_session_7526197867548014752_consolidate-financial-equations-7526197867548014752 (1)/distilled_equations_final.csv"
