...
- optimal dividend / payout policy
- important aspects
- cdo tranches
- debt obligations
- remaining principal
...
- many approaches
- related problem
- single-stock execution problems
- principal payments
- 4-6 defaults
tags_extracted: '2025-12-18T17:54:28.243878'
...
$$
V_{L} = V + \mathrm{PV} (\text{Debt tax shield}) \tag{2}
$$
...
$$
r = r_{D} \frac{D}{V} + r_{E} \frac{E}{V} \tag{3}
$$
...
$$
r^{*} = r_{D} \left(1 - T_{c}\right) \frac{D}{V} + r_{E} \frac{E}{V} \tag{4}
$$
...
$$
X_{i} = \sqrt{\rho} M + \sqrt{1 - \rho} Z_{i} \tag{5}
$$
...
$$
\begin{array}{l} \mathrm{P} (\text{Credit} i \text{defaults} | M) = \mathrm{P} (X_{i} \leq \bar{x}_{i} | M) \\ = \mathrm{P} (\sqrt{\rho} M + \sqrt{1 - \rho} Z_{i} \leq \Phi^{-1} (q) \mid M) \\ = \quad \mathrm{P} \left(Z_{i} \leq \frac{\Phi^{-1} (q) - \sqrt{\rho} M}{\sqrt{1 - \rho}} \mid M\right). \\ \end{array}
$$
...
$$
p (k \mid M) = \left( \begin{array}{c} N \\ k \end{array} \right) q_{M}^{k} (1 - q_{M})^{N - k}.
$$
...
$$
\mathrm{P} (k \text{defaults}) = \int_{-\infty}^{\infty} p (k \mid M) \phi (M) d M
$$
...
$$
\mathrm{E}_{0}^{Q} [ \text{Equity tranche loss} ] = 3 \times \mathrm{P} (3 \text{or more defaults}) + \sum_{k = 1}^{2} k \mathrm{P} (k \text{defaults})
$$

$$
\mathrm{E}_{0}^{Q} [ \text{Mezzanine tranche loss} ] = 3 \times \mathrm{P} (6 \text{or more defaults}) + \sum_{k = 1}^{2} k \mathrm{P} (k + 3 \text{defaults})
$$

$$
\mathrm{E}_{0}^{Q} [ \text{Senior tranche loss} ] = \sum_{k = 1}^{119} k \mathrm{P} (k + 6 \text{defaults}).
$$
...
$$
M_{k} = (1 + c) M_{k - 1} - B \quad \text{for} k = 1, 2, \dots , n \tag{7}
$$

with  $M_{n} = 0$ . We can iterate (7) to obtain

$$
\begin{array}{l} M_{k} = (1 + c)^{k} M_{0} - B \sum_{p = 0}^{k - 1} (1 + c)^{p} \\ = (1 + c)^{k} M_{0} - B \left[ \frac{(1 + c)^{k} - 1}{c} \right]. \tag{8} \\ \end{array}
$$

But  $M_{n} = 0$  and so we obtain

$$
B = \frac{c (1 + c)^{n} M_{0}}{(1 + c)^{n} - 1}. \tag{9}
$$

We can now substitute (9) back into (8) and obtain

$$
M_{k} = M_{0} \frac{(1 + c)^{n} - (1 + c)^{k}}{(1 + c)^{n} - 1}.
$$
...
$$
\begin{array}{l} F_{0} = \sum_{k = 1}^{n} \frac{B}{(1 + r)^{k}} \\ = \frac{c (1 + c)^{n} M_{0}}{(1 + c)^{n} - 1} \times \frac{(1 + r)^{n} - 1}{r (1 + r)^{n}}. \tag{10} \\ \end{array}
$$
...
$$
\begin{array}{l} \mathrm{P} \& \mathrm{L} \quad \approx \quad \delta \Delta S + \frac{\Gamma}{2} (\Delta S)^{2} + \text{vega} \Delta \sigma \\ = \text{deltaP} \& L + \text{gammaP} \& L + \text{vegaP} \& L. \tag{13} \\ \end{array}
$$
...
$$
\begin{array}{l} \mathrm{P} \& \mathrm{L} \quad \approx \quad \delta S \left(\frac{\Delta S}{S}\right) + \frac{\Gamma S^{2}}{2} \left(\frac{\Delta S}{S}\right)^{2} + \text{vega} \Delta \sigma \\ = \text{ESP} \times \text{Return} + \mathbb{S} \text{Gamma} \times \text{Return}^{2} + \text{vega} \Delta \sigma \tag{14} \\ \end{array}
$$
...
$$
\mathrm{VaR}_{\alpha} = \mu + \sigma \Phi^{-1} (\alpha) \quad \text{where} \Phi \text{is the standard normal CDF.} \tag{15}
$$
...
$$
q_{\alpha} \left(F_{L}\right) > q_{\alpha} \left(F_{L_{1}}\right) + q_{\alpha} \left(F_{L_{1}}\right) \quad \text{is possible}. \tag{16}
$$
...
$$
\widehat{S}_{k} = S_{k} - h \left(n_{k}\right). \tag{17}
$$

We model the permanent price impact via the function,  $g(n)$ , so that

$$
S_{k + 1} = S_{k} + \sigma z_{k} - g \left(n_{k}\right) \tag{18}
$$
...
$$
\begin{array}{l} \sum_{k = 1}^{T} \widehat{S}_{k} n_{k} = \sum_{k = 1}^{T} \left(S_{k} - h (n_{k})\right) n_{k} \\ = \sum_{k = 1}^{T} \left(S_{1} + \sum_{j = 1}^{k - 1} (\sigma z_{j} - g (n_{j})\right) n_{k} - \sum_{k = 1}^{T} h (n_{k}) n_{k} (19) \\ = S_{1} X + \sigma \sum_{k = 1}^{T} z_{k} x_{k} - \sum_{k = 1}^{T} g \left(n_{k}\right) x_{k} - \sum_{k = 1}^{T} h \left(n_{k}\right) n_{k}. (20) \\ \end{array}
$$
