...
$$
(1) \qquad \tilde {S} _ {0} ^ {j} = S _ {0} ^ {j} / S _ {0} ^ {1} \qquad \mathrm {a n d} \qquad \tilde {S} _ {1} ^ {j} (\omega) = S _ {1} ^ {j} (\omega) / S _ {1} ^ {1} (\omega).
$$
...
$$
0 = \left(S _ {1} (-) - S _ {0}\right) (\pi (- +) + \pi (- -)) + \left(S _ {1} (+) - S _ {0}\right) (\pi (+ +) + \pi (+ -)) \quad \Longrightarrow
$$
...
$$
V _ {0} (\text {C A L L}) = \sum_ {n = n _ {*}} ^ {T} \left(S _ {0} d ^ {T} (u / d) ^ {n} - K\right) \binom {T} {n} p ^ {n} q ^ {T - n}. \tag {30}
$$
...
$$
V _ {0} \left(\mathrm {C A L L}\right) = E \left(S _ {0} e ^ {Y} - K\right) + \tag {33}
$$
