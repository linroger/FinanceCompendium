...
$$
\begin{array}{l} M^{*}(t) = \max_{0\leq s\leq t}W^{*}(s) \\ = \max  _ {0 \leqslant s \leqslant t} a W (s / a ^ {2}) \\ = a M \left(t / a ^ {2}\right). \\ \end{array}
$$
...
$$
\mathcal {F} _ {t} := \sigma \left(\{W (r) \} _ {0 \leq r \leq s}\right) \quad \text {a n d} \quad \mathcal {F} _ {\infty} := \sigma \left(\{W (r) \} _ {0 \leq r <   \infty}\right); \tag {20}
$$
