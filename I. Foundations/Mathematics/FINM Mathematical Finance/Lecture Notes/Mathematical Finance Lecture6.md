...
$$
\frac{d S_{t}}{S_{t}} = \mu d t + \sigma d W_{t}, \tag{1}
$$
...
$$
\theta(s) = \sum_{j = 0}^{m - 1} \xi_{j} \mathbf{1}_{(t_{j}, t_{j + 1}]}(s). \tag{2}
$$
...
$$
\int \theta_{s} d W_{s} = \sum_{j = 0}^{m - 1} \xi_{j} (W(t_{j + 1}) - W(t_{j})) \tag{3}
$$
...
$$
E \left(\int \theta_{s} d W_{s}\right) = 0 \tag{4}
$$

$$
E \left(\int \theta_{s} d W_{s}\right)^{2} = \int E \theta_{s}^{2} d s \tag{5}
$$
...
$$
\int \left(a \theta_{s} + b \eta_{s}\right) d W_{s} = a \int \theta_{s} d W_{s} + b \int \eta_{s} d W_{s}. \tag{6}
$$
...
$$
\lim_{n \to \infty} \int E (\theta_{s} - \theta_{s}^{(n)})^{2} d s = 0. \tag{8}
$$
...
$$
\int \theta_{s} d W_{s} = \lim_{n \rightarrow \infty} \int \theta_{s}^{(n)} d W_{s} \tag{9}
$$
...
$$
E | \int \theta_{s}^{(n)} d W_{s} - \int \theta_{s}^{(m)} d W_{s} |^{2} = \int E | \theta_{s}^{(n)} - \theta_{s}^{(m)} |^{2} d s <   4 \varepsilon .
$$
...
$$
\lim_{n \to \infty} \int \theta_{s}^{(n)} d W_{s}
$$
...
$$
\int_{a}^{b} \theta_{s} d W_{s} := \int \theta_{s}^{(a, b ]} d W_{s} \tag{10}
$$

where  $\{\theta_s^{(a,b]}\}$  is the adapted  $\mathcal{H}^2$  function defined by

$$
\begin{array}{l} \theta_{s}^{(a, b ]} = \theta_{s} \quad \text{if } \quad a <   s \leq b \\ = 0 \qquad \text{otherwise}. \\ \end{array}
$$
...
$$
I_{t} = E \left(\int \theta_{s} d W_{s} \mid \mathcal{F}_{t}\right). \tag{11}
$$
...
$$
T_{n} = \min \{t \geq 0: \int_{0}^{t} \theta(s)^{2} d s = n \},
$$

with the convention that  $T_{n} = \infty$  if there is no such  $t$ , and define an adapted process  $\theta^{(n)}(t)$

$$
\theta^{(n)}(t) = \theta(t) \quad \text{for} \quad t \leq T_{n} \quad \text{and}
$$

$$
\theta^{(n)}(t) = 0 \quad \text{for} \quad t > T_{n}
$$
...
$$
E \int_{0}^{\infty} \theta^{(n)}(s)^{2} d s = E \int_{0}^{T_{n}} \theta(s)^{2} d s \leq E n = n <   \infty .
$$
...
$$
\int \theta(s) d W_{s} = \lim_{n \rightarrow \infty} \int \theta^{(n)}(s) d W_{s}. \tag{13}
$$
...
$$
\int_{0}^{\infty} E \theta_{s}^{2} d s = \int_{0}^{1} E W_{s}^{2} d s = \int_{0}^{1} s d s = \frac{1}{2} <   \infty .
$$
...
$$
\begin{array}{l} \int_{0}^{\infty} E \left(\theta_{s} - \theta_{s}^{(n)}\right)^{2} d s = \sum_{k = 0}^{2^{n} - 1} \int_{k / 2^{n}}^{(k + 1) / 2^{n}} E \left(\theta_{s} - \theta_{s}^{(n)}\right)^{2} d s \\ = \sum_{k = 0}^{2^{n} - 1} \int_{k / 2^{n}}^{(k + 1) / 2^{n}} E \left(W_{s} - W_{k / 2^{n}}\right)^{2} d s \\ = \sum_{k = 0}^{2^{n} - 1} \int_{k / 2^{n}}^{(k + 1) / 2^{n}} (s - (k / 2^{n})) d s \\ \leq \sum_{k = 0}^{2^{n} - 1} 2^{- 2 n} = 2^{n} / 2^{2 n} \longrightarrow 0 \\ \end{array}
$$
...
$$
\int \theta_{s}^{(n)} d W_{s} = \sum_{k = 0}^{2^{n} - 1} W_{k / 2^{n}} (W_{(k + 1) / 2^{n}} - W_{k / 2^{n}}).
$$
...
$$
\begin{array}{l} W_{1}^{2} = \sum_{k = 0}^{2^{n} - 1} \left(W_{(k + 1) / 2^{n}}^{2} - W_{k / 2^{n}}^{2}\right) \\ = \sum_{k = 0}^{2^{n} - 1} \left(W_{(k + 1) / 2^{n}} - W_{k / 2^{n}}\right) \left(W_{(k + 1) / 2^{n}} + W_{k / 2^{n}}\right) \\ = \sum_{k = 0}^{2^{n} - 1} \left(W_{(k + 1) / 2^{n}} - W_{k / 2^{n}}\right) \left(W_{k / 2^{n}} + W_{k / 2^{n}}\right) \\ + \sum_{k = 0}^{2^{n} - 1} \left(W_{(k + 1) / 2^{n}} - W_{k / 2^{n}}\right) \left(W_{(k + 1) / 2^{n}} - W_{k / 2^{n}}\right) \\ = 2 \sum_{k = 0}^{2^{n} - 1} W_{k / 2^{n}} \left(W_{(k + 1) / 2^{n}} - W_{k / 2^{n}}\right) \\ + \sum_{k = 0}^{2^{n} - 1} \left(W_{(k + 1) / 2^{n}} - W_{k / 2^{n}}\right)^{2} \\ \end{array}
$$
...
$$
\boxed {\int_{0}^{1} W_{s} d W_{s} = \frac{1}{2} \left(W_{1}^{2} - 1\right).} \tag {15}
$$

Note that if the Ito integral obeyed the Fundamental Theorem of Calculus, then the value of the integral would be

$$
\int_{0}^{t} W_{s} d W_{s} = \int_{0}^{t} W (s) W^{\prime} (s) d s = \frac{W_{s}^{2}}{2} \Bigg |_{0}^{t} = \frac{W_{t}^{2}}{2}
$$
...
$$
u (W_{t}, t) - u (0, 0) = \int_{0}^{t} u_{x} \left(W_{s}, s\right) d W_{s} + \int_{0}^{t} u_{t} \left(W_{s}, s\right) d s + \frac{1}{2} \int_{0}^{t} u_{x x} \left(W_{s}, s\right) d s. \tag {16}
$$
...
$$
W_{t}^{2} - t = 2 \int_{0}^{t} W_{s} d W_{s}, \tag {18}
$$
...
$$
u_{x} (x, t) = \alpha u (x, t), \quad \text{an d}
$$

$$
u_{x x} (x, t) = \alpha^{2} u (x, t).
$$

Thus, the Itô formula implies

$$
u \left(W_{t}, t\right) = 1 + \alpha \int_{0}^{t} u \left(W_{t}, t\right) d W_{t} + \left(\beta + \frac{1}{2} \alpha^{2}\right) \int_{0}^{t} u \left(W_{t}, t\right) d t. \tag {20}
$$

Set  $S_{t} = u(W_{t}, t)$ ; then equation (20), in differential form, becomes

$$
\frac{d S_{t}}{S_{t}} = \alpha d W_{t} + \left(\beta + \frac{1}{2} \alpha^{2}\right) d t. \tag {21}
$$
...
$$
\frac{d S_{t}}{S_{t}} = \alpha d W_{t}, \qquad \mathrm{eq ui va le nt ly},
$$

$$
\exp \left\{\theta W_{t} - \theta^{2} t / 2 \right\} = 1 + \theta \int_{0}^{t} \exp \left\{\theta W_{s} - \theta^{2} s / 2 \right\} d W_{s}.
$$
...
$$
Z_{t} = Z_{0} + \int_{0}^{t} A_{s} d s + \int_{0}^{t} B_{s} d W_{s} \tag {22}
$$
...
$$
d Z_{t} = A_{t} d t + B_{t} d W_{t}. \tag {23}
$$
...
$$
\int_{0}^{t} \theta_{s} d Z_{s} = \int_{0}^{t} \theta_{s} A_{s} d s + \int_{0}^{t} \theta_{s} B_{s} d W_{s}. \tag {24}
$$
...
$$
\begin{array}{l} d U_{t} = u_{x} \left(Z_{t}, t\right) d Z_{t} + u_{t} \left(Z_{t}, t\right) d t + \frac{1}{2} u_{x x} \left(Z_{t}, t\right) B_{t}^{2} d t \tag {25} \\ = u_{x} (Z_{t}, t) B_{t} d W_{t} + \left(u_{x} (Z_{t}, t) A_{t} + \frac{1}{2} u_{x x} (Z_{t}, t) B_{t}^{2} + u_{t} (Z_{t}, t)\right) d t \\ \end{array}
$$
...
$$
\begin{array}{l} \int_{0}^{t} A_{s} d s = E \left(\int_{0}^{t + u} A_{s} d s \mid \mathcal {F}_{t}\right) \quad \Longrightarrow \\ \int_{0}^{t} A_{s} d s = E \left(\int_{0}^{t} A_{s} d s + \int_{t}^{t + u} A_{s} d s \mid \mathcal {F}_{t}\right) \quad \Longrightarrow \\ \int_{0}^{t} A_{s} d s = \int_{0}^{t} A_{s} d s + E \left(\int_{t}^{t + u} A_{s} d s \mid \mathcal {F}_{t}\right) \quad \Longrightarrow \\ 0 = E \left(\int_{t}^{t + u} A_{s} d s \mid \mathcal {F}_{t}\right) \quad \Longrightarrow \\ 0 = \int_{t}^{t + u} E \left(A_{s} \mid \mathcal {F}_{t}\right) d s \quad \Longrightarrow \\ 0 = \frac{1}{u} \int_{t}^{t + u} E (A_{s} \mid \mathcal {F}_{t}) d s \\ \end{array}
$$
...
$$
\lim_{u \to 0 +} \frac{1}{u} \int_{t}^{t + u} E (A_{s} | \mathcal {F}_{t}) d s = E (A_{t} | \mathcal {F}_{t}) = A_{t}
$$
...
$$
\int_{0}^{t} f (s) d W_{s} = f (t) W (t) - \int_{0}^{t} f^{\prime} (s) W_{s} d s.
$$
...
$$
Z_{t} = u \left(W_{t}, t\right) \quad \text{fo r} 0 \leq t \leq T.
$$
...
$$
u (x, t) = E u \left(W_{T - t} + x, T\right).
$$
...
$$
\lim_{n \rightarrow \infty} \sum_{j = 0}^{2^{n} - 1} g \left(W \left(t_{j}\right), t_{j}\right)\left(\left(W \left(t_{j + 1}\right) - W \left(t_{j}\right)\right)^{2} = \int_{0}^{1} g \left(W_{t}, t\right) d t \quad a n d \right. \tag {26}
$$

$$
\lim_{n \rightarrow \infty} \sum_{j = 0}^{2^{n} - 1} o \left(\left(\left(W \left(t_{j + 1}\right) - W \left(t_{j}\right)\right)^{2}\right) = 0 \right. \tag {27}
$$
...
$$
\begin{array}{l} u (W_{1}, 1) - u (0, 0) = \sum_{j = 1}^{2^{n}} \left(u (W (t_{j}), t_{j}) - u (W (t_{j - 1}), t_{j - 1})\right) \\ = \sum_{j = 1}^{2^{n}} \left(u \left(W (t_{j}), t_{j}\right) - u \left(W (t_{j}), t_{j - 1}\right)\right) \\ + \sum_{j = 1}^{2^{n}} (u (W (t_{j}), t_{j - 1}) - u (W (t_{j - 1}), t_{j - 1})) \\ := S_{1}^{(n)} + S_{2}^{(n)}. \\ \end{array}
$$
...
$$
\begin{array}{l} u (W (t_{j}), t_{j}) - u (W (t_{j}), t_{j - 1}) = u_{t} (W (t_{j}), t_{j}) (t_{j} - t_{j - 1}) + o (t_{j} - t_{j - 1}) \\ = u_{t} (W (t_{j}), t_{j}) 2^{- n} + o (2^{- n}). \\ \end{array}
$$
...
$$
S_{1}^{(n)} = 2^{- n} \sum_{j = 1}^{2^{n}} u_{t} (W (t_{j}), t_{j}) + o (1) \longrightarrow \int_{0}^{1} u_{s} (W_{s}, s) d s.
$$
...
$$
\begin{array}{l} u \left(W \left(t_{j}\right), t_{j - 1}\right) - u \left(W \left(t_{j - 1}\right), t_{j - 1}\right) \\ = u_{x} \left(W \left(t_{j - 1}\right), t_{j - 1}\right) \left(W \left(t_{j}\right) - W \left(t_{j - 1}\right)\right) \\ + (1 / 2) u_{x x} \left(W \left(t_{j - 1}\right), t_{j - 1}\right) \left(W \left(t_{j}\right) - W \left(t_{j - 1}\right)\right)^{2} \\ + o \left(\left(W \left(t_{j}\right) - W \left(t_{j - 1}\right)\right)^{2}\right). \\ \end{array}
$$

By Lemma 2 and the definition of the Itô integral, as  $n \to \infty$ ,

$$
\begin{array}{l} \sum_{j = 1}^{2^{n}} u_{x} (W (t_{j - 1}), t_{j - 1}) (W (t_{j}) - W (t_{j - 1})) \longrightarrow \int_{0}^{1} u_{x} (W (s), s) d W_{s}, \\ \sum_{j = 1}^{2^{n}} u_{x x} \left(W \left(t_{j - 1}\right), t_{j - 1}\right) \left(W \left(t_{j}\right) - W \left(t_{j - 1}\right)\right)^{2} \longrightarrow \int_{0}^{1} u_{x x} \left(W_{s}, s\right) d s, \text{an d} \\ \sum_{j = 1}^{2^{n}} o \big ((W (t_{j}) - W (t_{j - 1}))^{2} \big) \longrightarrow 0. \\ \end{array}
$$

It follows that

$$
\begin{array}{l} u (W_{1}, 1) - u (0, 0) = \lim_{n \rightarrow \infty} \left(S_{1}^{(n)} + S_{2}^{(n)}\right) \\ = \int_{0}^{1} u_{s} (W_{s}, s) d s + \int_{0}^{1} u_{x} (W (s), s) d W_{s} \\ + \frac{1}{2} \int_{0}^{1} u_{x x} (W_{s}, s) d s. \\ \end{array}
$$
...
$$
| s - t | <   \delta \quad \Longrightarrow \quad | g (W_{t}, t) - g (W_{s}, s) | <   \varepsilon .
$$
...
$$
\begin{array}{l} \sum_{j = 0}^{2^{n} - 1} g \left(W \left(t_{j}\right), t_{j}\right) \left(\left(W \left(t_{j + 1}\right) - W \left(t_{j}\right)\right)^{2} \right. \tag {28} \\ = \sum_{i = 0}^{2^{m} - 1} g (W (i / 2^{m})) \left(\sum_{j = 0}^{2^{n - m} - 1} ((W ((i / 2^{m}) + (j / 2^{n})) - W ((i / 2^{m}) + (j / 2^{n}) - (1 / 2^{n})))^{2}\right) + \text{er ro r}, \\ \end{array}
$$
...
$$
\lim_{m \rightarrow \infty} 2^{- m} \sum_{i = 0}^{2^{m} - 1} g (W (i / 2^{m})) = \int_{0}^{1} g (W_{t}, t) d t
$$
