...
$$
\mathbb {E} \left(R _ {i}\right) = \mathbb {E} \left(R _ {Z P}\right) + \beta_ {i P} \left[ \mathbb {E} \left(R _ {P}\right) - \mathbb {E} \left(R _ {Z P}\right) \right] \forall i = 1, \dots , N \tag {1}
$$
...
$$
\mathbb {E} \left(R _ {i}\right) = R _ {f} + \beta_ {i M} \left[ \mathbb {E} \left(R _ {M}\right) - R _ {f} \right] \forall i = 1, \dots , N ^ {\quad 1} \tag {2}
$$

$$
\frac {\partial V a r \left(R _ {p}\right)}{\partial \omega_ {i}} = C o v \left(R _ {i}, R _ {p}\right)
$$

So we can think of CAPM in the following way:

$$
\mathbb {E} \left(R _ {i}\right) - R _ {f} = \underbrace {C o v \left(R _ {i} , R _ {m}\right)} _ {\text {c o n t r i b u t i o n o f i t o p o r t f o l i o r i s k}} \quad \times \underbrace {\frac {\mathbb {E} \left(R _ {m}\right) - R _ {f}}{V a r \left(R _ {f}\right)}}
$$
...
$$
\mathbb {E} \left(\widetilde {R} _ {i}\right) = \gamma + \sum_ {i = 1} ^ {K} \underbrace {\beta_ {i k}} _ {\text {f a c t o r s e n s .}} \times \underbrace {\lambda_ {k}} _ {\text {f a c t o r r i s k p r}} \quad \forall i \tag {3}
$$

$$
\widetilde {R} _ {i} = a _ {i} + \sum_ {i = 1} ^ {K} \underbrace {\beta_ {i k}} _ {\text {f a c t o r s e n s .}} \times \underbrace {\widetilde {f} _ {k}} _ {\text {r i s k f a c t o r}} + \widetilde {\varepsilon} _ {i} \quad \forall i \tag {4}
$$

$$
\text {w i t h} \mathbb {E} (\widetilde {\varepsilon} _ {i}) = 0 \text {a n d} \mathbb {E} \left(\widetilde {f} _ {k} \widetilde {\varepsilon} _ {i}\right) \equiv C o v \left(\widetilde {f} _ {k}, \widetilde {\varepsilon} _ {i}\right) = 0 \tag {5}
$$
...
$$
\text {f a c t o r - m i m i c k i n g p a y o f f s :} f ^ {*} = \operatorname {p r o j} (f | \underline {{X}}) \tag {6}
$$

$$
\text {f a c t o r - m i m i c k i n g g r o s s r e t u r n s :} f ^ {*} = \frac {\operatorname {p r o j} (f | \underline {{X}})}{p (\operatorname {p r o j} (f | \underline {{X}}))} \tag {7}
$$

$$
\text {f a c t o r - m i m i c k i n g e x c e s s r e t u r n s :} f ^ {*} = \operatorname {p r o j} (f | \underline {{R}} ^ {e}) \tag {8}
$$
...
$$
\mathbb {E} \left(\widetilde {R} _ {i}\right) - R _ {f} = \beta_ {i} \mathbb {E} (\widetilde {f}) \tag {10}
$$

where  $\beta_{i}$  defined through he time-series regression

$$
R _ {i, t} - R _ {f, t} = \alpha_ {i} + \beta_ {i} \times f _ {t} + \varepsilon_ {i, t} \tag {11}
$$
...
$$
\mathbb {E} \left(\widetilde {R} _ {i}\right) - R _ {f} = \gamma_ {i} + \beta_ {i} \lambda \tag {13}
$$
...
$$
\operatorname {r e g r e s s} R _ {i t} - R _ {f t} = \alpha_ {i} + \beta_ {i M} \left(R _ {M, t} - R _ {f t}\right) + \varepsilon_ {i t} \tag {14}
$$
...
$$
\operatorname {r e g r e s s} \overline {{R}} _ {i} - \overline {{R}} _ {f} = \gamma + \hat {\beta} _ {i M} \lambda + u _ {i} \tag {15}
$$
