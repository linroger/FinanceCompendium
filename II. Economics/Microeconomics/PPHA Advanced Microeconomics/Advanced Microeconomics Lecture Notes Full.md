---
aliases:
  - Economics and Public Policy General Ideas
tags:
key_concepts:
parent_directory: PPHA Advanced Microeconomics
cssclasses: academia
title: "Lecture 1 - Economics and Public Policy: General Ideas"
linter-yaml-title-alias: "Lecture 1 - Economics and Public Policy: General Ideas"
---
...
Here I outline different models that can be used to understand the properties of equilibrium employment and wages.
...
$$
\boldsymbol {f} _ {i} ^ {\prime} \left(L _ {i} ^ {d}\right) = w \tag {2}
$$
...
$$
u _ {j} \left(y _ {j}, I _ {j}\right) = u _ {j} \left(w L _ {j}, T - L _ {j}\right) \tag {3}
$$

The first order condition for utility maximization is

$$
\begin{array}{l} \frac {\partial u _ {j} \left(w L _ {j} , T - L _ {j}\right)}{\partial y} w - \frac {\partial u _ {j} \left(w L _ {j} , T - L _ {j}\right)}{\partial l} = 0 \Rightarrow \tag {4} \\ \frac {\partial u _ {j} \left(y _ {j} , I _ {j}\right)}{\partial y} w = \frac {\partial u _ {j} \left(y _ {j} , I _ {j}\right)}{\partial I} \\ \end{array}
$$
...
Next, consider the same population of firms and workers, but assume that the government fixes wages at  $\overline{w}$ . Unless  $\overline{w} = w^c$ , then the level of labor will not be such that demand equals supply. In other words, there will be imbalance between supply and demand at the fixed price. How is the level of employment determined in this case? A common approach is to assume the "short side rule" under which the level of labor is determined by the minimum of supply and demand, i.e.

$$
L = \min  (D (w), S (w)) \tag {6}
$$

The idea of the rule is that no one firm is forced to hire somebody at the market wage nor is any worker forced to work at the market wage.

Suppose that the wage level is set such that  $\overline{w} > w^c$ . Interpret this as a government imposed minimum wage. In a competitive labor market, supply will exceed demand at this fixed wage. Under the short side rule, it is evident that fewer workers are employed than occurs when wages clear the labor market. This is the classic objection to minimum wage laws, i.e. that the laws decrease employment. Notice that the average wellbeing of workers may increase or decrease in the case. Why? Even if the number of workers decreases, the wages of those who are working will be higher. Hence, a policy evaluation, using worker wellbeing as the evaluative criterion, will depend on the sensitivity of labor demand to wages.

# Case 3: monopsony

Suppose that case 1 is modified so that there is a single firm that sets the wage; this is called monopsony (as opposed to monopoly) since the demand side of the market (as opposed to the supply side) has market power. In this case, the firm will account for the fact that the more workers hired, the higher the wage that must be paid to each. In other words, given labor supply function  $S(w)$ , the firm can determine the necessary wage to hire  $L$  workers by inverting the function. This will be the inverse of the supply function, call this inverse  $w(L)$ . The idea is that if the monopsonist knows the supply of workers at wage  $w$ , the monopsonist then knows the smallest wage that must be paid to hire  $L$  workers and so will account for this in wage setting.

In this case, the monopsonist chooses  $L$  to maximize

$$
f (L) - w (L) L \tag {7}
$$

which implies that level of employment chosen by the monopsonist,  $L^{m}$  is determined by the condition

$$
f ^ {\prime} \left(L ^ {m}\right) = w \left(L ^ {m}\right) + w ^ {\prime} \left(L ^ {m}\right) L ^ {m} \tag {8}
$$

The effects of market power can be seen by comparing the first order conditions (2) and (8). In (2) the firm does not account for the effect of wages on the labor supply, whereas this is accounted for in equation (8) through the term  $w'(L)L$ . It is straightforward to see, in comparing (2) and (8) that  $L^m < L^c$  and  $w^m < w^c$ ; the monopsonist hires fewer workers in order to pay a lower wage, than would occur in a competitive environment. In this case, there is no involuntary unemployment, but this masks the fact that wages are set by the firm at the lowest level needed to produce its desired labor level.

Now, consider a government policy which sets the wage level at a  $\bar{w}$  such that  $w^{m} < \bar{w} \leq w^{c}$ . Given this rule, the firm will now maximize

$$
f (L) - \bar {w} L \tag {9}
$$

Call the profit maximizing labor level  $\bar{L}$ . It must be the case that  $\bar{L} > L^m$ . Why? A monopsonistic firm sets wages below marginal product because it is accounting for how wages affect labor supply. If the wage is exogenously set, the monopsonist no longer accounts for the effects of wages on labor supply in determining the level of labor to hire. The monopsonist no longer has market power over the wage level. In this context, implementation of a minimum wage raises the level of employment as well as the wage. Workers are unambiguously better off.
...
Suppose a policymaker needs to make a policy choice  $p$  out of a set of possible policies  $P$ . There are data  $d$  available to the policymaker. These data tell us something about the environment for which the choice is being made. The policy decision is analogous to that of the individual I have described as underlying economic models. Each policy entails some set of socioeconomic outcomes.  $o(p, d)$ . Why do outcomes depend on the data? Data include things such as the state of the economy. The effects of a tax

change on the distribution of income will exhibit such dependence. Each of the outcomes generates a benefit  $B(o,p,d)$  and a cost  $C(o,p,d)$ . Note that I allow the benefit and the cost to depend on the data. For example, the benefit of a policy that raises consumption (the outcome) may be assessed differently during a recession as opposed to a boom. The policy problem may be described as maximizing the difference between the benefit and the cost of a policy, i.e.

$$
\max  _ {p \in P} B (o (p, d), p, d) - C (o (p, d), p, d). \tag {15}
$$

Where does economic theory come into play? One answer involves the formulation of  $o(\cdot, \cdot)$ . The effects of a policy are determined by the ways that individuals make choices and by the associated way that the outcomes of interest are determined in equilibrium. Yet theory also matters in the formulation of  $B(o, p, d)$  and  $C(o, p, d)$ . Evaluation of outcomes, benefits and costs, depends on how one thinks the world works.

As described, this first problem assumed that the policymaker knew the effect of the policy on outcomes with certainty. It would be wonderful if this were the typical case. The more realistic case is the following. The policymaker cannot know the effect with certainty, but each policy alters the likelihood of different outcomes. The policymaker has data  $d$ . Each policy, formally, produces a conditional probability for outcomes  $\mu(o|p,d)$ . The dependence of the conditional probability on data reflects the fact that data comprise part of the information available to the policymaker in assessing the effects of policy in an uncertain world.

What is the decision problem under uncertainty for the policymaker? This depends on how the policymaker reacts to uncertainty. One standard assumption is that a policymaker should, in the presence of uncertainty, maximize the difference between the expected benefit and the expected cost. In this case, the decision problem is

$$
\max  _ {p \in P} \int_ {o} B (o (p, d), p, d) \mu (o | p, d) - \int_ {o} C (o (p, d), p, d) \mu (o | p, d) \tag {16}
$$

Economic theory now matters because it may allow the policymaker to better model  $\mu(o|p,d)$  as well as  $B(o,p,d)$  and  $C(o,p,d)$ . Note that this formulation also indicates the role of statistical models in policy work: they involve determining  $\mu(o|p,d)$ .
...
$$
\sum_ {i} u \left(c _ {i}\right) \tag {2.1}
$$
...
$$
s (c) = s \left(u \left(c _ {1}\right), \dots , u \left(c _ {I}\right)\right) \tag {2.3}
$$
...
$$
s (c) = \max  _ {i} u \left(c _ {i}\right) \tag {2.4}
$$
...
$$
s (c) = \min  _ {i} u \left(c _ {i}\right) \tag {2.5}
$$
...
$$
\forall i u \left(\boldsymbol {c} _ {i} ^ {1}\right) \geq u \left(\boldsymbol {c} _ {i} ^ {2}\right) \text {a n d f o r a t l e a s t o n e} i, u \left(\boldsymbol {c} _ {i} ^ {1}\right) > u \left(\boldsymbol {c} _ {i} ^ {2}\right)
$$
...
$$
s (c) = \sum_ {i} \varphi \left(u \left(c _ {i}\right)\right) \tag {2.6}
$$
...
$$
\omega_ {i} = \phi \left(X _ {i}, Z _ {i}, \varepsilon_ {i}\right) \tag {3.1}
$$
...
$$
\forall i, j \mu \left(\omega_ {i} \mid X _ {i}, Z _ {i}\right) = \mu \left(\omega_ {j} \mid X _ {j}, Z _ {j}\right) \text {i f} X _ {i} = X _ {j} \tag {3.2}
$$
...
$$
J (x) = J \left(x _ {1}, \dots , x _ {I}\right) \tag {3.4}
$$
...
$$
\forall i u _ {i} \left(x _ {i}\right) = u _ {i} \left(\bar {x} _ {i}\right) \tag {3.6}
$$
...
$$
\frac {d f \left(x ^ {*} , p\right)}{d x _ {k}} = 0 \text {f o r} k = 1 \dots K \tag {4.17}
$$
...
$$
D ^ {2} f \left(x ^ {*}\right) \text {i s a n e g a t i v e d e f i n i t e m a t r i x}. \tag {4.18}
$$
...
$$
\max  _ {x} f (x) \text {s u c h t h a t} g (x) = y \tag {4.20}
$$
...
$$
\max  _ {x, \lambda} f (x) - \lambda (g (x) - y) \tag {4.21}
$$
...
$$
\forall k \frac {\partial f (x)}{\partial x _ {k}} - \lambda \frac {\partial g (x)}{\partial x _ {k}} = 0. \tag {4.22}
$$
...
$$
\frac {d L ^ {*} (y)}{d y} = \sum_ {k = 1} ^ {K} \left(\frac {\partial f (x ^ {*})}{\partial x _ {k}} \frac {d x _ {k} ^ {*}}{d y} - \lambda^ {*} \frac {\partial g (x ^ {*})}{\partial x _ {k}} \frac {d x _ {k} ^ {*}}{d y}\right) - \frac {d \lambda^ {*}}{d y} (g (x ^ {*}) - y) + \lambda^ {*} =
$$

$$
\sum_ {k = 1} ^ {K} \left(\left(\frac {\partial f (x ^ {*})}{\partial x _ {k}} - \lambda^ {*} \frac {\partial g (x)}{\partial x _ {k}}\right) \frac {d x _ {k} ^ {*}}{d y}\right) + \lambda^ {*} = \tag {4.25}
$$
...
$$
\max  _ {x} f (x) \text {s u c h t h a t} g (x) \leq y \tag {4.26}
$$
...
$$
\max  _ {c, \lambda} u (c) - \lambda (p c - I) \tag {4.28}
$$

First order conditions are

$$
\forall k \frac {\partial u (\boldsymbol {c})}{\partial \boldsymbol {c} _ {k}} = \lambda p _ {k} \tag {4.29}
$$
...
$$
\operatorname {v a r} (x) = E \left(\left(x - E (x)\right) ^ {2}\right) = E \left(x ^ {2}\right) - E (x) ^ {2} \tag {5.3}
$$
...
$$
\operatorname {s d} (x) = \sqrt {\operatorname {v a r} (x)} \tag {5.4}
$$
...
$$
\operatorname {c o v} (x y) = E ((x - E (x)) (y - E (y))) = E (x y) - E (x) E (y) \tag {5.5}
$$
...
$$
\operatorname {c o r r} (x y) = \frac {\operatorname {c o v} (x y)}{\operatorname {s d} (x) \operatorname {s d} (y)} \tag {5.6}
$$
...
$$
\operatorname {v a r} (x) = E (\operatorname {v a r} (x | y)) + \operatorname {v a r} (E (x | y)) \tag {5.10}
$$

Here is a proof.

$$
\begin{array}{l} E \left(\operatorname {v a r} (x | y)\right) = E \left(E \left(x ^ {2} | y\right)\right) - E \left(E (x | y) ^ {2}\right) = \tag {5.11} \\ E \left(x ^ {2}\right) - E \left(E (x | y) ^ {2}\right) \\ \end{array}
$$

$$
\begin{array}{l} \operatorname {v a r} \left(E (x | y)\right) = E \left(E (x | y) ^ {2}\right) - \left(E (E (x | y))\right) ^ {2} = \tag {5.12} \\ E \left(E (x | y) ^ {2}\right) - E (x) ^ {2} \\ \end{array}
$$
...
$$
\text {I f} f (x) \text {i s c o n c a v e ,} E (f (x)) \leq f (E (x)) \tag {5.13}
$$
...
$$
\text {I f} f (x) \text {i s c o n c a v e ,} p f (\bar {x}) + (1 - p) f (\bar {\bar {x}}) \leq f (p \bar {x} + (1 - p) \bar {\bar {x}}) \tag {5.14}
$$
...
$$
E \left(R \beta \frac {u ^ {\prime} \left(c _ {1}\right)}{u ^ {\prime} \left(c _ {0}\right)}\right) = 1 \tag {5.24}
$$

Using the definition of a covariance, (5.24) can be rewritten<sup>2</sup>

$$
\operatorname {c o v} \left(R \beta \frac {u ^ {\prime} \left(c _ {1}\right)}{u ^ {\prime} \left(c _ {o}\right)}\right) + E (R) E \left(\beta \frac {u ^ {\prime} \left(c _ {1}\right)}{u ^ {\prime} \left(c _ {o}\right)}\right) = 1 \tag {5.25}
$$

which implies

$$
E (R) = \frac {1 - \operatorname {c o v} \left(R \beta \frac {u ^ {\prime} \left(c _ {1}\right)}{u ^ {\prime} \left(c _ {o}\right)}\right)}{E \left(\beta \frac {u ^ {\prime} \left(c _ {1}\right)}{u ^ {\prime} \left(c _ {o}\right)}\right)} \tag {5.26}
$$
...
$$
E \left(\beta R _ {f} \frac {u ^ {\prime} \left(c _ {1}\right)}{u ^ {\prime} \left(c _ {o}\right)}\right) = 1 \tag {5.27}
$$

which implies

$$
R _ {f} = \frac {1}{E \left(\beta \frac {u ^ {\prime} \left(c _ {1}\right)}{u ^ {\prime} \left(c _ {o}\right)}\right)} \tag {5.28}
$$
...
$$
E (R) = \left(1 - \operatorname {c o v} \left(R \beta \frac {u ^ {\prime} \left(c _ {1}\right)}{u ^ {\prime} \left(c _ {o}\right)}\right)\right) R _ {f} \tag {5.29}
$$

which produces the formula

$$
E (R) - R _ {f} = - R _ {f} \operatorname {c o v} \left(R \beta \frac {u ^ {\prime} \left(c _ {1}\right)}{u ^ {\prime} \left(c _ {o}\right)}\right) \tag {5.30}
$$
...
$$
E (R) - R _ {f} = - \frac {\operatorname {c o v} \left(R u ^ {\prime} \left(c _ {1}\right)\right)}{E \left(u ^ {\prime} \left(c _ {1}\right)\right)} \tag {5.31}
$$
...
1. For all  $\overline{c} \in C$ , the sets  $\{c \text{ such that } c \preceq \overline{c}\}$  and  $\{c \text{ such that } c \succeq \overline{c}\}$  are closed.
...
$$
u (\boldsymbol {c}) \leq u (\boldsymbol {c} ^ {\prime}) \leq u (\boldsymbol {c} ^ {\prime \prime}) \mathrm {i m p l i e s} u (\boldsymbol {c}) \leq u \left(\lambda \boldsymbol {c} ^ {\prime} + (1 - \lambda) \boldsymbol {c} ^ {\prime \prime}\right), \lambda \in [ 0, 1 ]
$$
...
$$
\left\{c \text {s u c h t h a t} u (c) = \bar {u} \right\} \tag {1}
$$
...
$$
\sum_ {k} p _ {k} c _ {k} = p ^ {\prime} c \leq y \tag {3}
$$
...
$$
y = \sum_ {t = 1} ^ {T} \frac {1}{R ^ {t - 1}} I _ {t} \tag {5}
$$
...
$$
p ^ {\prime} c = \sum_ {t = 1} ^ {T} \frac {1}{R ^ {t - 1}} c _ {t} \tag {6}
$$

Together, (5) and (6) produce an intertemporal budget constraint

$$
\sum_ {t = 1} ^ {T} \frac {1}{R ^ {t - 1}} I _ {t} = \sum_ {t = 1} ^ {T} \frac {1}{R ^ {t - 1}} c _ {t} \tag {7}
$$
...
$$
\max  _ {c, \lambda} u (c) + \lambda \left(y - p ^ {\prime} c\right) \tag {8}
$$
...
$$
\min  _ {c} p ^ {\prime} c \text {s u c h t h a t} u (c) \geq u \Leftrightarrow \tag {10}
$$

$$
\min _ {c} L = p ^ {\prime} c + \lambda (u - u (c))
$$
...
$$
\frac {d e (p , u)}{d p _ {i}} = h _ {i} (p, u) \tag {12}
$$
...
$$
\begin{array}{l} \frac {\partial x _ {i} (p , \boldsymbol {e} (p , u))}{\partial p _ {i}} + \frac {\partial x _ {i} (p , \boldsymbol {e} (p , u))}{\partial y} \frac {\partial \boldsymbol {e} (p , u)}{\partial p _ {i}} = \\ \frac {\partial x _ {i} (p , \boldsymbol {e} (p , u))}{\partial p _ {i}} + \frac {\partial x _ {i} (p , \boldsymbol {e} (p , u))}{\partial y} h _ {i} (p, u) = \tag {15} \\ \begin{array}{c} \frac {\partial h _ {i} (p , u)}{\partial p _ {j}} \end{array} \\ \end{array}
$$
...
$$
\frac {\partial x _ {i} (p , \boldsymbol {e} (p , u))}{\partial p _ {i}} = \frac {\partial h _ {i} (p , u)}{\partial p _ {j}} - \frac {\partial x _ {i} (p , \boldsymbol {e} (p , u))}{\partial y} x _ {i} (p, \boldsymbol {e} (p, u)) \tag {16}
$$

Since  $e(p, u) = y$  when a consumer has maximized utility

$$
\frac {\partial x _ {i} (p , y)}{\partial p _ {i}} = \frac {\partial h _ {i} (p , u)}{\partial p _ {i}} - \frac {\partial x _ {i} (p , y)}{\partial y} x _ {i} (p, y) \tag {17}
$$
...
$$
i \mathbf {c} _ {i 1} = \pi_ {i} \frac {y _ {i}}{p _ {1}} \tag {18}
$$
...
$$
\max  _ {c _ {1}, c _ {2}} \mathbf {c} _ {1} ^ {\alpha_ {i}} \mathbf {c} _ {2} ^ {1 - \alpha_ {i}} \text {s u c h t h a t} p _ {1} \mathbf {c} _ {1} + p _ {2} \mathbf {c} _ {2} = y _ {i} \tag {19}
$$
...
$$
\min  _ {c \in C} E (I (\theta , d, c) | d) = \min  _ {c \in C} \int I (\theta , d, c) \mu (\theta | d) \tag {7.1}
$$
...
$$
\min  _ {c \in C} \max  _ {\theta} I (\theta , d, c) \tag {7.4}
$$
...
$$
r (\theta , d, c) = I (\theta , d, c) - \min  _ {c \in C} I (\theta , d, c) \tag {7.5}
$$
...
$$
\min  _ {c} \max  _ {\theta} r (\theta , d, c) \tag {7.6}
$$
...
$$
\min  _ {c \in C} \left(\alpha \max  _ {\theta} I (\theta , d, c) + (1 - \alpha) E (I (\theta , d, c) | d)\right) \tag {7.7}
$$
...
$$
\Phi_ {i} \left(x _ {i}, x _ {- i}\right) \tag {1}
$$
...
$$
\frac {\partial \Phi_ {i} \left(x _ {i} , x _ {- i}\right)}{\partial x _ {j b}} \geq 0, j \neq i \tag {2}
$$
...
$$
\frac {\partial^ {2} \Phi_ {i} (x _ {i} , x _ {- i})}{\partial x _ {i a} \partial x _ {j b}} \geq 0, j \neq i \tag {3}
$$
...
$$
\begin{array}{l} \partial \frac {\partial \Phi_ {i} (x _ {i} , x _ {- i})}{\partial x _ {i a}} \\ \hline \partial x _ {j b} \end{array} \geq 0  .
$$
...
$$
\begin{array}{l} \Phi_ {i} \left(X _ {i a} ^ {\prime \prime}, X _ {j b} ^ {\prime \prime}, X _ {i, - a}, X _ {- i, - j b}\right) - \Phi_ {i} \left(X _ {i a} ^ {\prime}, X _ {j b} ^ {\prime \prime}, X _ {i, - a}, X _ {- i, - j b}\right) \geq \\ \Phi_ {i} \left(X _ {i a} ^ {\prime \prime}, X _ {j b} ^ {\prime}, X _ {i, - a}, X _ {- i, - j b}\right) - \Phi_ {i} \left(X _ {i a} ^ {\prime}, X _ {j b} ^ {\prime}, X _ {i, - a}, X _ {- i, - j b}\right) \tag {5} \\ \end{array}
$$
...
$$
\left(x ^ {\prime} \wedge x ^ {\prime \prime}\right) _ {r} = \min  \left\{x _ {r} ^ {\prime}, x _ {r} ^ {\prime \prime} \right\} \tag {6}
$$
...
$$
\left(x ^ {\prime} \vee x ^ {\prime \prime}\right) _ {r} = \max  \left\{x _ {r} ^ {\prime}, x _ {r} ^ {\prime \prime} \right\}. \tag {7}
$$
...
$$
0 \text {i f} \omega_ {t} \leq 9
$$

$$
. 9 \omega_ {i t} \text {i f} 9 <   \omega_ {i t} \leq 1 0 \tag {33}
$$

$$
\omega_ {i t} \text {i f} 1 0 <   \omega
$$

and

$$
f _ {2} \left(\omega_ {i t}, \omega_ {i t}\right) =
$$
...
$$
H _ {i, c, t} = h \left(I _ {i, c, t}, \zeta_ {i, c, t}\right) \tag {1}
$$
...
$$
\operatorname {c o v} \left(\zeta_ {i, c, t}, \zeta_ {i ^ {\prime}, c, t}\right) = 0 \text {i f} i \neq i ^ {\prime}
$$
...
$$
\log Y _ {i, a, t + 1} = f \left(H _ {i, c, t}, \varepsilon_ {i, a, t + 1}\right) \tag {2}
$$
...
$$
\operatorname {C O V} \left(\varepsilon_ {i, a, t} \varepsilon_ {i, a, t ^ {\prime}}\right) = 0 \text {i f} t \neq t ^ {\prime}
$$

$$
\operatorname {C O V} \left(\varepsilon_ {i, a, t} \varepsilon_ {i ^ {\prime}, a, t ^ {\prime}}\right) = 0 \text {i f} i \neq i ^ {\prime}
$$

$$
\operatorname {C O V} \left(\varepsilon_ {i, a, t} \zeta_ {i ^ {\prime}, c, t ^ {\prime}}\right) = 0 \quad \forall i, i ^ {\prime}, t, t ^ {\prime}
$$
...
$$
Y _ {i, a, t} = C _ {i, a, t} + I _ {i, c, t} \tag {3}
$$
...
$$
I _ {i, c, t} = g \left(\log Y _ {i, a, t}, \zeta_ {i, c, t}\right) \tag {4}
$$
...
$$
\mathcal {H} _ {i, c, t} = h \left(\mathbf {g} \left(\log Y _ {i, a, t}, \zeta_ {i, c, t}\right), \zeta_ {i, c, t}\right) \tag {5}
$$

while substituting (5) into (2) produces

$$
\log Y _ {i, a, t + 1} = f \left(h \left(g \left(\log Y _ {i, a, t}, \zeta_ {i, c, t}\right), \zeta_ {i, c, t}\right), \varepsilon_ {i, a, t + 1}\right) \tag {6}
$$
...
$$
\log Y _ {i, a, t + 1} = \alpha + \beta \log Y _ {i, a, t} + e _ {i, t + 1} \tag {7}
$$
...
$$
\boldsymbol {H} _ {i, c, t} = \theta \log I _ {i, c, t} + \zeta_ {i, c, t} \tag {8}
$$

with

$$
\zeta_ {i, c, t} = \delta + \lambda \zeta_ {i, c, t - 1} + v _ {i, a, t} \tag {9}
$$
...
$$
\log Y _ {i, a, t + 1} = \mu + p H _ {i, c, t} + \varepsilon_ {i, a, t + 1}. \tag {10}
$$

Third, the utility of the adult  $i$  born at time  $t - 1$  is

$$
U _ {i, a, t} = (1 - \pi) \log C _ {i, a, t} + \pi \log Y _ {i, a, t + 1}. \tag {11}
$$

Together, these produce a law of motion for income

$$
\log Y _ {i, a, t + 1} = \kappa + p \theta \log Y _ {i, a, t} + p \varsigma_ {i, c, t} + \varepsilon_ {i, a, t + 1} \tag {12}
$$
...
$$
H _ {i, c, t} = h \left(I _ {i, c, t}, H _ {i, c, t - 1}, \zeta_ {i, c, t}\right) \tag {15}
$$
