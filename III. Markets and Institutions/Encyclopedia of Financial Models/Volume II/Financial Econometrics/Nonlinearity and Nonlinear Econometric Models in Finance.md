...
$$ x _ {t} = \mu + \sum_ {i = 0} ^ {\infty} \psi_ {i} a _ {t - i} \tag {1}
...
$$ x _ {t} = f \left(a _ {t}, a _ {t - i}, \dots\right) \tag {2}
$$
...
$$
\mu_ {t} = E \left(x _ {t} \mid F _ {t - i}\right) \equiv g \left(F _ {t - i}\right) \tag {3}
$$

$$
\sigma_ {t} ^ {2} = \operatorname {V a r} \left(x _ {t} \mid F _ {t - 1}\right) \equiv h \left(F _ {t - 1}\right)
$$ where  $g(.)$  and  $h(.)$  are well-defined functions with  $h(.) > 0$ . Thus, we restrict the model to


$$ x _ {t} = g \left(F _ {t - 1}\right) + \sqrt {h \left(F _ {t - 1}\right)} e _ {t}
...
$$
\begin{array}{l} x _ {t} = c + \sum_ {\substack {i = 1 \\ m - s}} ^ {p} \phi_ {i} x _ {t - i} - \sum_ {j = 1} ^ {q} \theta_ {j} a _ {t - j} \tag{4} \\ + \sum_ {i = 1} ^ {m} \sum_ {j = 1} ^ {s} \beta_ {i j} x _ {t - i} a _ {t - j} + a _ {t} \\ \end{array}
...
$$ x _ {t} = \mu + \sum_ {i = 1} ^ {s} \beta_ {i} a _ {t - i} a _ {t} + a _ {t} \tag {5}
$$ where  $\{a_t\}$  is a white noise series. The first two conditional moments of  $x_{t}$  are


$$
\begin{array}{l} E \left(x _ {t} \mid F _ {t - 1}\right) = \mu \\ \mathrm {V a r} (x _ {t} | F _ {t - 1}) = \left(1 + \sum_ {i = 1} ^ {s} \beta_ {i} a _ {t - i}\right) ^ {2} \sigma_ {a} ^ {2} \\ \end{array}
...
$$
\begin{array}{l} R _ {t} = \mu + \phi_ {1} R _ {t - 1} + \phi_ {3} R _ {t - 3} \\ + (1 + \beta_ {1} a _ {t - 1} + \beta_ {3} a _ {t - 3}) a _ {t} \\ \end{array}
...
$$
\begin{array}{l} R _ {t} = 0. 0 1 1 4 + 0. 1 6 7 R _ {t - 1} - 0. 0 9 5 R _ {t - 3} \\ + 0. 0 7 1 (1 + 0. 3 7 7 a _ {t - 1} - 0. 6 4 6 a _ {t - 3}) \epsilon_ {t} \\ \end{array}
...
$$
\hat {\epsilon} _ {t} = \frac {R _ {t} - 0 . 0 1 1 4 - 0 . 1 6 7 R _ {t - 1} + 0 . 0 9 5 R _ {t - 3}}{0 . 0 7 1 (1 + 0 . 3 7 7 \hat {a} _ {t - 1} - 0 . 6 4 6 \hat {a} _ {t - 3})}
...
$$
\begin{array}{l} R _ {t} = 0. 0 1 3 + 0. 2 2 3 R _ {t - i} + 0. 0 0 6 R _ {t - 2} \\ - 0. 0 1 3 R _ {t - 3} + a _ {t} \\ \end{array}
$$

$$
\begin{array}{l} \sigma_ {t} ^ {2} = 0. 0 0 2 + 0. 1 8 5 a _ {t - 1} ^ {2} + 0. 3 0 1 a _ {t - 2} ^ {2} \tag {7} \\ + 0. 1 9 7 a _ {t - 3} ^ {2} \\ \end{array}
...
$$ x _ {t} = \left\{ \begin{array}{l l} - 1. 5 x _ {t - 1} + a _ {t} & \text {i f} x _ {t - 1} <   0 \\ 0. 5 x _ {t - 1} + a _ {t} & \text {i f} x _ {t - 1} \geq 0 \end{array} \right. \tag {8}
...
$$
\begin{array}{l} x _ {t} = \phi_ {0} ^ {(j)} + \phi_ {0} ^ {(j)} x _ {t - 1} - \dots \phi_ {p} ^ {(j)} x _ {t - p} + a _ {t} ^ {(j)} \tag {9} \\ i f \quad \gamma_ {j - 1} \leq x _ {t - d} <   \gamma_ {j} \\ \end{array}
...
$$
\begin{array}{l} (1 - 1. 1 3 B + 0. 2 7 B ^ {2}) (1 - 0. 5 1 B ^ {1 2}) y t \\ = (1 - 1. 1 2 B + 0. 4 4 B ^ {2}) \left(1 - 0. 8 2 B ^ {1 2}\right) a _ {t} \tag {10} \\ \end{array}
...
$$ y _ {t} = \left\{ \begin{array}{c c} 0. 0 8 3 y _ {t - 2} + 0. 1 5 8 y _ {t - 3} + 0. 1 1 8 y _ {t - 4} \\ - 0. 1 8 0 y _ {t - 1 2} + a _ {1 t} & \text {i f} y _ {t - i} \leq 0. 1 \\ 0. 4 2 1 y _ {t - 2} + 0. 2 3 9 y _ {t - 3} - 0. 1 2 7 y _ {t - 1 2} \\ + a _ {2 t} & \text {i f} y _ {t - i} > 0. 1 \end{array} \right. \tag {11}
...
$$ r _ {t} = 0. 0 5 8 + a _ {t}, \quad a _ {t} = \sigma_ {t} \epsilon_ {t} \tag {12}
$$

$$
\sigma_ {t} ^ {2} = 0. 0 4 1 + 0. 0 9 3 a _ {t - 1} ^ {2} + 0. 8 9 4 \sigma_ {t - 1} ^ {2}
...
$$
\begin{array}{l} r _ {t} = 0. 0 1 5 + a _ {t}, \quad a _ {t} = \sigma_ {t} \epsilon_ {t} \\ \sigma_ {t} ^ {2} = 0. 0 3 2 + 0. 0 3 3 a _ {t - 1} ^ {2} + 0. 0 9 1 N _ {t - 1} a _ {t - 1} ^ {2} \\ + 0. 9 1 1 \sigma_ {t - 1} ^ {2} \tag {13} \\ \end{array}
...
$$ r _ {t} = 0. 0 2 3 + a _ {t}, a t = \sigma_ {t} \epsilon_ {t}
$$

$$
\begin{array}{l} \sigma_ {t} ^ {2} = 0. 0 8 6 + 0. 0 4 4 a _ {t - 1} ^ {2} + 0. 8 1 5 \sigma_ {t - 1} ^ {2} \\ + (- 0. 1 1 4 + 0. 0 5 2 a _ {t - 1} ^ {2} + 0. 2 1 4 \sigma_ {t - 1} ^ {2}) N _ {t - 1} \tag {14} \\ \end{array}
...
$$
\begin{array}{l} x _ {t} = c _ {0} + \sum_ {i = 1} ^ {p} \phi_ {0, i} x _ {t - i} + F \left(\frac {x _ {t - d} - \Delta}{s}\right) \tag {15} \\ \times \left(c _ {1} + \sum_ {i = 1} ^ {p} \phi_ {1, i} x _ {t - i}\right) + a _ {t} \\ \end{array}
...
$$
\mu_ {1 t} = c _ {0} + \sum_ {i = 1} ^ {p} \phi_ {0, i} x _ {t - i}
$$

$$
\mu_ {2 t} = \left(c _ {0} + c _ {1}\right) + \sum_ {i = 1} ^ {p} \left(\phi_ {0, i} + \phi_ {1, i}\right) x _ {t - i}
$$
...
$$
R _ {t} = 0. 0 1 3 + a _ {t}, \quad a _ {t} = \sigma_ {t} \epsilon_ {t} \tag {16}
$$

$$
\sigma_ {t} ^ {2} = 0. 0 0 3 + 0. 0 8 8 a _ {t - 1} ^ {2} + 0. 1 0 9 a _ {t - 2} ^ {2}
...
$$
\begin{array}{l} R _ {t} = 0. 0 1 5 + a t, \quad a _ {t} = \sigma_ {t} \epsilon_ {t} \\ \sigma_ {t} ^ {2} = \left(0. 0 0 3 + 0. 2 0 5 a _ {t - 1} ^ {2} + 0. 0 9 2 a _ {t - 2} ^ {2}\right) \tag {17} \\ + \frac {0 . 0 0 1 - 0 . 2 3 9 a _ {t - 1} ^ {2}}{1 + \exp (- 1 0 0 0 a _ {t - 1})} \\ \end{array}
...
$$
\sigma_ {t} ^ {2} = 0. 0 0 3 + 0. 2 0 5 a _ {t - 1} ^ {2} + 0. 0 9 2 a _ {t - 2} ^ {2}
$$

Yet for a large positive  $a_{t - 1}$ , the volatility process behaves like the ARCH(2) model

$$
\sigma_ {t} ^ {2} = 0. 0 0 4 - 0. 0 3 4 a _ {t - 1} ^ {2} + 0. 0 9 2 a _ {t - 2} ^ {2}
$$
...
$$
\left\{ \begin{array}{l l} c _ {1} + \sum_ {i = 1} ^ {p} \phi_ {1, i} x _ {t - i} + a _ {1 t} & \text {i f} s _ {t} = 1 \\ c _ {2} + \sum_ {i = 1} ^ {p} \phi_ {2, i} x _ {t - i} + a _ {2 t} & \text {i f} s _ {t} = 2 \end{array} \right. \tag {18}
$$ where  $s_t$  assumes values in  $\{1,2\}$  and is a first-order Markov chain with transition probabilities


$$
P (s _ {t} = 2 | s _ {t - i} = 1) = w _ {1}, \quad P (s _ {t} = 1 | s _ {t - 1} = 2) = w _ {2}
$$
...
$$
Y _ {t} = m \left(X _ {t}\right) + a _ {t} \tag {19}
...
$$ y _ {t} = m (x) + a _ {t}, \quad t = 1, \ldots , T
$$
...
$$
\hat {m} (x) = \frac {1}{T} \sum_ {\mathrm {t} = 1} ^ {\mathrm {T}} w _ {t} (x) y _ {t} \tag {20}
...
$$
K _ {h} (x) = \frac {1}{h} K (x / h), \quad \int K _ {h} (z) d z = 1 \tag {21}
$$

The weight function can now be defined as

$$ w _ {t} (x) = \frac {K _ {h} \left(x - x _ {t}\right)}{\sum_ {t = 1} ^ {T} K _ {h} \left(x - x _ {t}\right)} \tag {22}
...
$$
\hat {m} (x) = \sum_ {t = 1} ^ {T} w _ {t} (x) y _ {t} = \frac {\sum_ {t = 1} ^ {T} K _ {h} \left(x - x _ {t}\right) y _ {t}}{\sum_ {t = 1} ^ {T} K _ {h} \left(x - x _ {t}\right)} \tag {23}
...
$$
K _ {h} (x) = \frac {1}{h \sqrt {2 \pi}} \exp \left(- \frac {x ^ {2}}{2 h ^ {2}}\right)
...
$$
\hat {m} (x _ {t}) \rightarrow \frac {\sum_ {t = 1} ^ {T} K _ {h} (0) y _ {t}}{\sum_ {t = 1} ^ {T} K _ {h} (0)} = \frac {1}{T} \sum_ {t = 1} ^ {T} y _ {t} = \bar {y}
...
$$
C V (h) = \frac {1}{T} \sum_ {j = 1} ^ {T} [ y _ {j} - \hat {m} _ {h, j} (x _ {j}) ] ^ {2} W (x _ {j})
...
$$
L (a, b) = \sum_ {t = 1} ^ {T} \left[ y _ {t} - a - b \left(x - x _ {t}\right) \right] ^ {2} K _ {h} \left(x - x _ {t}\right) \tag {24}
...
$$
\begin{array}{l} \sum_ {t = 1} ^ {T} K _ {h} \left(x - x _ {t}\right) y _ {t} = a \sum_ {t = 1} ^ {T} K _ {h} \left(x - x _ {t}\right) \\ + b \sum_ {t = 1} ^ {T} (x - x _ {t}) K _ {h} (x - x _ {t}) \\ \end{array}
$$

$$
\begin{array}{l} \sum_ {t = 1} ^ {T} y _ {t} \left(x - x _ {t}\right) K _ {h} \left(x - x _ {t}\right) = a \sum_ {t = 1} ^ {T} \left(x - x _ {t}\right) K _ {h} \left(x - x _ {t}\right) \\ + b \sum_ {t = 1} ^ {T} (x - x _ {t}) ^ {2} K _ {h} (x - x _ {t}) \\ \end{array}
$$
...
$$
\begin{array}{l} \left[ \begin{array}{c c} s _ {T, 0} & s _ {T, 1} \\ s _ {T, 1} & s _ {T, 2} \end{array} \right] \left[ \begin{array}{c} a \\ b \end{array} \right] \\ = \left[ \begin{array}{c} \sum_ {t = 1} ^ {T} K _ {h} (x - x _ {t}) y _ {t} \\ \sum_ {t = 1} ^ {T} (x - x _ {t}) K _ {h} (x - x _ {t}) y _ {t} \end{array} \right] \\ \end{array}
$$

Consequently, we have

$$
\begin{array}{l} \hat {a} = \\ \frac {s _ {T , 2} \sum_ {t = 1} ^ {T} K _ {h} (x - x _ {t}) y _ {t} - s _ {T , 1} \sum_ {t = 1} ^ {T} (x - x _ {t}) K _ {h} (x - x _ {t}) y _ {t}}{s _ {T , 0} s _ {T , 2} - s _ {T , 1} ^ {2}} \\ \end{array}
$$

The numerator and denominator of the prior fraction can be further simplified as

$$
\begin{array}{l} s _ {T, 2} = \sum_ {t = 1} ^ {T} K _ {h} (x - x _ {t}) y _ {t} \\ - s _ {T, 1} \sum_ {t = 1} ^ {T} (x - x _ {t}) K _ {h} (x - x _ {t}) y _ {t} \\ = \sum_ {t = 1} ^ {T} \left[ K _ {h} \left(x - x _ {t}\right) \left(s _ {T, 2} - \left(x - x _ {t}\right) s _ {T, 1}\right) \right] y t. \\ \end{array}
$$

$$
\begin{array}{l} s _ {T, 0} s _ {T, 2} - s _ {T, 1} ^ {2} = \sum_ {t = 1} ^ {T} K _ {h} (x - x _ {t}) s _ {T, 2} \\ - \sum_ {t = 1} ^ {T} (x - x _ {t}) K _ {h} (x - x _ {t}) s _ {T, 1} \\ = \sum_ {t = 1} ^ {T} K _ {h} (x - x _ {t}) [ (s _ {T, 2} - (x - x _ {t}) s _ {T, 1}) ] \\ \end{array}
$$

In summary, we have

$$
\hat {a} = \frac {\sum_ {t = 1} ^ {T} w _ {t} y _ {t}}{\sum_ {t = 1} ^ {T} w _ {t}} \tag {25}
...
$$
\hat {m} (x) = \frac {\sum_ {t = 1} ^ {T} w _ {t} y _ {t}}{\sum_ {t = 1} ^ {T} w _ {t} + 1 / T ^ {2}} \tag {26}
$$
...
$$
\sum_ {t = 1} ^ {T} (y _ {t} - a) ^ {2} K _ {h} (x - x _ {t})
...
$$
K _ {h} (x) = \frac {1}{(h \sqrt {2 \pi}) ^ {p} | \boldsymbol {\Sigma} | ^ {1 / 2}} \exp \left(- \frac {1}{2 h ^ {2}} x ^ {\prime} \Sigma^ {- 1} x\right)
...
$$
K _ {h} (x) = \prod_ {i = 1} ^ {p} \frac {0 . 7 5}{h _ {i}} \left(1 - \frac {x _ {i} ^ {2}}{h _ {i} ^ {2}}\right) I \left(| \frac {x _ {i}}{h _ {i}} | <   1\right)
$$
...
$$ y _ {t} = \mu (x _ {t - 1}) d t + \sigma (x _ {t - 1}) d w _ {t}
...
$$ x _ {t} = f _ {1} \left(X _ {t - 1}\right) x _ {t - 1} + \dots + f _ {p} \left(X _ {t - 1}\right) x _ {t - p} + a _ {t} \tag {27}
...
$$ x _ {t} = f _ {0} (t) + \sum_ {i = 1} ^ {p} f _ {i} \left(x _ {t - i}\right) + a _ {t} \tag {28}
...
$$
S _ {t} = f _ {t} \left(S _ {t - 1}\right) + u _ {t}, \quad x _ {t} = g _ {t} \left(S _ {t}\right) + v _ {t} \tag {29}
...
$$ h _ {j} = f _ {j} \left(\alpha_ {0 j} + \sum_ {i \rightarrow j} w _ {i j} x _ {i}\right) \tag {30}
...
$$ h _ {j} = \frac {\exp \left(\alpha_ {0 j} + w _ {1 j} x _ {1} + w _ {2 j} x _ {2}\right)}{1 + \exp \left(\alpha_ {0 j} + w _ {1 j} x _ {1} + w _ {2 j} x _ {2}\right)}, \quad j = 1, 2, 3. \tag {31}
$$

For the output layer, the node is defined as

$$ o = f _ {o} \left(\alpha_ {0 o} + \sum_ {j \rightarrow o} w _ {j o} h _ {j}\right) \tag {32}
...
$$ o = \left\{ \begin{array}{l l} 1 & \text {i f} \alpha_ {0 o} + w _ {1 o} h _ {1} + w _ {2 o} h _ {2} + w _ {3 o} h _ {3} > 0 \\ 0 & \text {i f} \alpha_ {0 o} + w _ {1 o} h _ {1} + w _ {2 o} h _ {2} + w _ {3 o} h _ {3} \leq 0 \end{array} \right.
$$ if  $f_{\mathrm{o}}(.)$  is a Heaviside function.


Combining the layers, the output of a feedforward neural network can be written as

$$ o = f _ {o} \left[ \alpha_ {0 o} + \sum_ {j \rightarrow o} w _ {j o} f _ {j} \left(\alpha_ {0 j} + \sum_ {i \rightarrow j} w _ {i j} x _ {i}\right)\right] \tag {33}
$$

If one also allows for direct connections from the input layer to the output layer, then the network becomes

$$
\begin{array}{l} o = f _ {o} \left[ \alpha_ {0 o} + \sum_ {i \rightarrow o} \alpha_ {i o} x _ {i} + \sum_ {j \rightarrow o} w _ {j o} f _ {j} \right. \tag {34} \\ \left. \times \left(\alpha_ {0 j} + \sum_ {i \rightarrow j} w _ {i j} x _ {i}\right)\right] \\ \end{array}
...
$$ o = \alpha_ {0 \mathrm {o}} + \sum_ {i = 1} ^ {2} \alpha_ {i o} x _ {i} + \sum_ {j = 1} ^ {3} w _ {j o} h _ {j}
...
$$
\begin{array}{l} \hat {r} _ {t} = 3. 2 2 - 1. 8 1 f _ {1} \left(r _ {t - 1}\right) - 2. 2 8 f _ {2} \left(r _ {t - 1}\right) \tag {35} \\ - 0. 0 9 r _ {t - 1} - 0. 0 5 r _ {t - 2} - 0. 1 2 r _ {t - 3} \\ \end{array}
$$ where  $r_{t - 1} = (r_{t - 1},r_{t - 2},r_{t - 3})$  and the two logistic functions are


$$
\begin{array}{l} f _ {1} \left(\boldsymbol {r} _ {t - 1}\right) = \\ \frac {\exp \left(- 8 . 3 4 - 1 8 . 9 7 r _ {t - 1} + 2 . 1 7 r _ {t - 2} - 1 9 . 1 7 r _ {t - 3}\right)}{1 + \exp \left(- 8 . 3 4 - 1 8 . 9 7 r _ {t - 1} + 2 . 1 7 r _ {t - 2} - 1 9 . 1 7 r _ {t - 3}\right)} \\ f _ {2} \left(\boldsymbol {r} _ {t - 1}\right) = \\ \end{array}
$$

$$
\frac {\exp (3 9 . 2 5 - 2 2 . 1 7 r _ {t - 1} - 1 7 . 3 4 r _ {t - 2} - 5 . 9 8 r _ {t - 3})}{1 + \exp (3 9 . 2 5 - 2 2 . 1 7 r _ {t - 1} - 1 7 . 3 4 r _ {t - 2} - 5 . 9 8 r _ {t - 3})}
$$
...
$$ r _ {t} = 1. 1 0 1 + 0. 0 7 7 r _ {t - 1} + a _ {t}, \quad \sigma_ {a} = 6. 6 1 \tag {36}
$$
...
$$
Q (m) = T (T + 2) \sum_ {i = 1} ^ {m} \frac {\hat {\rho} _ {i} ^ {2} (a _ {i} ^ {2})}{T - i}
...
$$ b _ {3} \left(w _ {1}, w _ {2}\right) = \frac {g}{4 \pi^ {2}} \Gamma \left[ - \left(w _ {1} + w _ {2}\right) \right] \Gamma \left(w _ {1}\right) \Gamma \left(w _ {2}\right) \tag {38}
...
$$
\begin{array}{l} b \left(w _ {1}, w _ {2}\right) = \frac {\left| b _ {3} \left(w _ {1} , w _ {2}\right) \right| ^ {2}}{p \left(w _ {1}\right) p \left(w _ {2}\right) p \left(w _ {1} + w _ {2}\right)} \tag {39} \\ = \text {c o n s t a n t} (w _ {1}, w _ {2}) \\ \end{array}
$$
...
$$
C _ {k} (\delta) = \lim  _ {T _ {k} \rightarrow \infty} \frac {2}{T _ {k} \left(T _ {k} - 1\right)} \sum_ {i <   j} I _ {\delta} \left(X _ {i}, X _ {j}\right) \tag {40}
...
$$
C _ {\ell} (\delta , T) = \frac {2}{T _ {k} (T _ {k} - 1)} \sum_ {i <   j} I _ {\delta} \left(X _ {i} ^ {*}, X _ {j} ^ {*}\right), \quad \ell = 1, k
...
$$
\begin{array}{l l} C _ {k} (\delta , T) \to [ C _ {1} (\delta) ] ^ {k} & \text {w i t h p r o b a b i l i t y 1}, \\ & \text {a s} \quad T \to \infty \end{array}
...
$$
\begin{array}{l} N (\delta , T) = \frac {6}{T _ {k} (T _ {k} - 1) (T _ {k} - 2)} \\ \sum_ {t <   s <   u} I _ {\delta} \left(x _ {t}, x _ {s}\right) I _ {\delta} \left(x _ {s}, x _ {u}\right) \\ \end{array}
$$

The BDS test statistic is then defined as

$$
D _ {k} (\delta , T) = \sqrt {T} \left\{C _ {k} (\delta , T) - \left[ C _ {1} (\delta , T) \right] ^ {k} \right\} / \sigma_ {k} (\delta , T) \tag {41}
...
$$ x _ {t} = \boldsymbol {X} _ {t - 1} ^ {\prime} \phi + a _ {t} \tag {42}
...
$$
\hat {a} _ {t} = \boldsymbol {X} _ {t - 1} ^ {\prime} \boldsymbol {\alpha} _ {1} + \boldsymbol {M} _ {t - 1} ^ {\prime} \boldsymbol {\alpha} _ {2} + v _ {t} \tag {43}
$$ where  $M_{t - 1} = (\hat{x}_t^2,\dots ,\hat{x}_t^{s + 1})'$  for some  $s\geq 1$  , and compute the least squares residuals


$$
\hat {v} _ {t} = \hat {a} _ {t} - X _ {t - 1} ^ {\prime} \hat {\alpha} _ {1} - M _ {t - 1} ^ {\prime} \hat {\alpha} _ {2}
...
$$
F = \frac {\left(S S R _ {0} - S S R _ {1}\right) / g}{S S R _ {1} / (T - p - g)} \quad \text {w i t h} \quad g = s + p + 1 \tag {44}
...
$$
\hat {x} _ {t} ^ {2} = \boldsymbol {X} _ {t - 1} ^ {\prime} \beta + u _ {t}
...
$$ x _ {t} = X _ {t - 1} ^ {\prime} \phi + M _ {t - 1} ^ {\prime} \pmb {\alpha} + e _ {t}
...
$$ x _ {t} = \phi_ {0} + \sum_ {i = 1} ^ {p} \phi_ {i} x _ {t - i} + a _ {t} \tag {45}
$$ whereas the alternative hypothesis  $H_{a}:x_{t}$  follows the SETAR model


$$ x _ {t} = \left\{ \begin{array}{l l} \phi_ {0} ^ {(1)} + \sum_ {i = 1} ^ {p} \phi_ {i} ^ {(1)} x _ {t - i} + a _ {1 t} & \text {i f} x _ {t - d} <   r _ {1} \\ \phi_ {0} ^ {(2)} + \sum_ {i = 1} ^ {p} \phi_ {i} ^ {(2)} x _ {t - i} + a _ {2 t} & \text {i f} x _ {t - d} \geq r _ {1} \end{array} \right. \tag {46}
...
$$ l (r _ {1}) = l _ {1} (r _ {1}; \hat {\phi} _ {1}, \hat {\sigma} _ {1} ^ {2}; \hat {\phi} _ {2}, \hat {\phi} _ {2} ^ {2}) - l _ {0} (\hat {\phi}, \hat {\phi} _ {a} ^ {2})
...
$$
\begin{array}{l} x _ {(j) + d} = \beta_ {0} + \sum_ {i = 1} ^ {p} \beta_ {i} x _ {(j) + d - i} + a _ {(j) + d}, \tag {47} \\ j = 1, \ldots , T - d \\ \end{array}
...
$$
\begin{array}{l} \hat {e} _ {(m + j) + d} = \alpha_ {0} + \sum_ {i = 1} ^ {p} \alpha_ {i} x _ {(m + j) + d - i} + v _ {t}. \tag {48} \\ j = 1, \dots , T - d - m \\ \end{array}
...
$$
\begin{array}{l} (1 - 0. 1 8 0 B + 0. 0 9 9 B ^ {3} - 0. 1 0 5 B ^ {9}) r _ {3 t} \\ = 0. 0 0 8 6 + a _ {3 t} \\ \end{array}
$$
...
$$
\begin{array}{l} (1 - 0. 0 9 8 B + 0. 1 1 1 B ^ {3} - 0. 0 8 8 B ^ {5}) r _ {4 t} \\ = 0. 0 0 7 8 + a _ {4 t} \\ \end{array}
$$
...
$$
X ^ {2} = \sum_ {i = 1} ^ {2} \sum_ {j = 1} ^ {2} \frac {\left(m _ {i j} - m _ {i 0} m _ {0 j} / m\right) ^ {2}}{m _ {i 0} m _ {0 j} / m}
...
$$
M S E (\ell) = \frac {1}{m} \sum_ {j = 0} ^ {m - 1} \left[ x _ {T + \ell + j} - x _ {T + j} (\ell) \right] ^ {2} \tag {49}
$$

$$
M A D (\ell) = \frac {1}{m} \sum_ {j = 0} ^ {m - 1} | x _ {T + \ell + j} - x _ {T + j} (\ell) | \tag {50}
$$

$$
M A P E (\ell) = \frac {1}{m} \sum_ {j = 0} ^ {m - 1} \left| \frac {x _ {T + j} (\ell)}{x _ {T + j + \ell}} - 1 \right| \tag {51}
...
$$
\begin{array}{l} (1 - 0. 3 1 B ^ {4}) (1 - 0. 6 5 B) \Delta x _ {t} = (1 - 0. 7 8 B ^ {4}) a _ {t}, \\ \hat {\sigma} _ {a} ^ {2} = 0. 0 9 0 \tag {52} \\ \end{array}
...
$$
\Delta x _ {t} = \left\{ \begin{array}{c} 0. 0 1 + 0. 7 3 \Delta x _ {t - 1} + 0. 1 0 \Delta x _ {t - 2} + a _ {1 t} \\ \text {i f} \Delta x _ {t - 2} \leq 0. 1, \\ 0. 1 8 + 0. 8 0 \Delta x _ {t - 1} - 0. 5 6 \Delta x _ {t - 2} + a _ {2 t} \\ \text {o t h e r w i s e} \end{array} \right. \tag {53}
$$
...
$$
\Delta x _ {t} = \left\{ \begin{array}{c} - 0. 0 7 + 0. 3 8 \Delta x _ {t - 1} - 0. 0 5 \Delta x _ {t - 2} + \epsilon_ {1 t} \\ \text {i f} s _ {t} = 1 \\ 0. 1 6 + 0. 8 6 \Delta x _ {t - 1} - 0. 3 8 \Delta x _ {t - 2} + \epsilon_ {2 t} \\ \text {i f} s _ {t} = 2 \end{array} \right. \tag {54}
$$
