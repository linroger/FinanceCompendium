...
Safety First Measures: Safety-First Measures {
  shape: rectangle
  style.fill: "#e8f5e9"
  style.stroke: "#388e3c"
}

Risk Measures -> Dispersion Measures: encompasses
Risk Measures -> Safety First Measures: encompasses
...
Mean Absolute Moment: Mean Absolute Moment {
  shape: oval
  style.fill: "#fff3e0"
}
...
Lower Partial Moment: Lower Partial Moment {
  shape: oval
  style.fill: "#f1f8e9"
}

Dispersion Measures -> {Mean Standard Deviation, Mean Absolute Deviation, Mean Absolute Moment, Gini Index, Mean Entropy, Mean Colog}
Safety First Measures -> {Value at Risk, Conditional Value at Risk, Lower Partial Moment}
```
...
$$ where  $q$  is the power index and  $t$  is the target rate of return.


Given a sample of return observations, we can approximate equation (10) as follows:

$$
\mathrm{LP M} (r_{p}, q) = \sqrt [ q ]{\frac{1}{T} \sum_{k = 1}^{T} \max \left(r_{p}^{(k)} - \bar {r}_{p} , 0\right)^{q}}
$$ where as before
