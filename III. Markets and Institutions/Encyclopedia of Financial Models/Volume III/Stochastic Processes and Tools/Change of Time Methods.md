...
$$ d X_{t} = \beta (t, X_{t}) d t + \gamma (t, X_{t}) d B (t)
...
$$ d S_{t} = - \alpha S_{t} d t + \sigma d W_{t}
$$

Then  $S_{t}$  may be presented in the following form using the change of time method:

$$
S_{t} = e^{- \alpha t} [ S_{0} + \tilde {W} (\phi_{t}^{- 1}) ]
$$ where  $\phi_t^{-1}$  satisfies


$$
\phi_{t}^{- 1} = \sigma^{2} \int_{0}^{t} (e^{\alpha s} (S_{0} + \tilde {W} (\phi_{s}^{- 1})))^{2} d s
$$
...
$$ d S_{t} = \alpha (b - S_{t}) d t + \sigma d W_{t}
$$

Then  $S_{t}$  may be presented in the following form using the change of time method

$$
S_{t} = e^{- \alpha t} [ \mathrm{S}_{0} - b + \tilde {W} (\phi_{t}^{- 1}) ]
$$ where  $\phi_t^{-1}$  satisfies


$$
\phi_{t}^{- 1} = \sigma^{2} \int_{0}^{t} (e^{\alpha s} (S_{0} - b + \tilde {W} (\phi_{s}^{- 1})) + b)^{2} d s
$$
...
$$
C (T) = S (0) \Phi \left(y_{+}\right) - e^{- r T} K \Phi \left(y_{-}\right) \tag {3}
$$ where
...
$$
\begin{array}{l} C (T) = e^{- r T} E_{P^{*}} [ f (T) ] \\ = e^{- r T} E_{P *} [ \max  (S (T) - K, 0) ] \tag {11} \\ \end{array}
$$
...
$$
\begin{array}{l} C (T) = e^{- r T} E_{P *} [ \max  (S (T) - K, 0) ] \\ = e^{- r T} \frac{1}{\sqrt{2 \pi}} \int_{- \infty}^{+ \infty} \max  [ S (0) e^{\sigma u \sqrt{T} + (r - \frac{\sigma^{2}}{2}) T} \\ - K, 0 ] e^{- \frac{u^{2}}{2}} d u \tag {12} \\ \end{array}
$$
...
$$ y_{0} = \frac{\ln \left(\frac{K}{S (0)}\right) - (r - \sigma^{2} / 2) T}{\sigma \sqrt{T}}.
$$
...
$$
\begin{array}{l} C (T) = \frac{1}{\sqrt{2 \pi}} \int_{y_{0}}^{+ \infty} S (0) e^{\sigma u \sqrt{T} - \frac{\sigma^{2} T}{2}} e^{- u^{2} / 2} d u \\ - K e^{- r T} \left[ 1 - \Phi (y_{0}) \right] \\ = \frac{S (0)}{\sqrt{2 \pi}} \int_{y_{0} - \sigma \sqrt{T}}^{+ \infty} e^{- u^{2} / 2} d u - K e^{- r T} [ 1 - \Phi (y_{0}) ] \\ = S (0) \left[ 1 - \Phi \left(y_{0} - \sigma \sqrt{T}\right) \right] - K e^{- r T} \left[ 1 - \Phi \left(y_{0}\right) \right] \\ = S (0) \Phi \left(y_{+}\right) - K e^{- r T} \Phi \left(y_{-}\right) \tag {14} \\ \end{array}
...
$$ d S_{t} = a \left(L - S_{t}\right) d t + \sigma S_{t} d W_{t} \tag {15}
...
$$ d S_{t} = a (L - S_{t}) d t + \sigma S_{t} d W_{t}
$$ in (15) has the following solution


$$
S_{t} = e^{- a t} [ S_{0} - L + \tilde {W} (\phi_{t}^{- 1}) ] + L
$$ where  $\tilde{W} (\phi_t^{-1})$  is a one-dimensional Wiener process and


$$
\phi_{t}^{- 1} = \sigma^{2} \int_{0}^{t} (S_{0} - L + \tilde {W} (\phi_{t}^{- 1}) + e^{a s} L)^{2} d s
...
$$ d S_{t} = a^{*} \left(L^{*} - S_{t}\right) d t + \sigma S_{t} d W_{t}^{*} \tag {16}
$$ where
...
$$
W_{t}^{*} = W_{t} + \lambda \int_{0}^{t} S (u) d u \tag {18}
...
$$
S_{t} = e^{- a^{*} t} \left[ S_{0} - L^{*} + \tilde {W}^{*} \left(\left(\phi_{t}^{*}\right)^{- 1}\right) \right] + L \tag {19}
...
$$
\phi_{t}^{*} = \sigma^{- 2} \int_{0}^{t} \left(S_{0} - L^{*} + \tilde {W}^{*} (s) + e^{a^{*} \phi_{s}^{*}} L^{*}\right)^{- 2} d s \tag {20}
$$

We note that

$$
\left(\phi^{*}\right)_{t}^{- 1} = \sigma^{2} \int_{0}^{t} \left(S_{0} - L^{*} + \tilde {W}^{*} \left(\left(\phi_{t}^{*}\right)^{- 1}\right) + e^{a^{*} s} L^{*}\right)^{2} d s
...
$$ f_{T} = \left(S_{T} - K\right) + := \max  \left(S_{T} - K, 0\right)
...
$$
\begin{array}{l} f_{T} = [ e^{- a T} (S_{0} - L + \bar {W}^{*} (\phi_{T}^{- 1})) + L - K ]^{+} \\ = [ S (0) e^{- a^{*} T} e^{\sigma W^{*} (T) - \frac{\sigma^{2} T}{2}} \\ + a^{*} L^{*} e^{- a^{*} T} e^{\sigma W^{*} (T) - \frac{\sigma^{2} T}{2}} \int_{0}^{T} e^{a^{*} s} e^{- \sigma W^{*} (s) \frac{\sigma^{2} s}{2}} \\ \times d s - K ]^{+} \tag {21} \\ \end{array}
$$
...
$$
\begin{array}{l} C_{T}^{*} = e^{- (r + a^{*}) T} S (0) \Phi (y_{+}) - e^{- r T} K \Phi (y_{-}) \\ + L^{*} e^{- (r + a^{*}) T} \left[ \left(e^{a^{*} T} - 1\right) - \int_{0}^{y 0} z F_{T}^{*} (d z) \right] \tag {22} \\ \end{array}
$$ where  $y_0$  is the solution of the following equation


$$
\begin{array}{l} y_{0} = \frac{\ln \left(\frac{K}{S (0)}\right) + \left(\frac{\sigma^{2}}{2} + a^{*}\right) T}{\sigma \sqrt{T}} \\ - \frac{\ln \left(1 + \frac{a^{*} L^{*}}{S (0)} \int_{0}^{T} e^{a^{*} s} e^{- \sigma y_{0} \sqrt{s} + \frac{\sigma^{2} s}{2}} d s\right)}{\sigma \sqrt{T}} \tag {23} \\ \end{array}
$$

$$ y^{+} := \sigma \sqrt{T} - y_{0} \quad \text{an d} \quad y_{-} := - y_{0},
$$
...
$$
\begin{array}{l} C_{T} := e^{- r T} E_{P *} f_{T} \\ = e^{- r T} E_{P^{*}} \left[ e^{- a^{*} T} \left(S_{0} - L + \tilde {W}^{*} \left(\phi_{T}^{- 1}\right)\right) + L^{*} - K \right]^{+} \\ = \frac{1}{\sqrt{2 \pi}} e^{- r T} \int_{- \infty}^{+ \infty} \max  [ S (0) e^{- a^{*} T} e^{\sigma y \sqrt{T} - \frac{\sigma^{2} T}{2}} \\ + a L e^{- a^{*} T} e^{\sigma y \sqrt{T} - \frac{\sigma^{2} T}{2}} \int_{0}^{T} e^{a^{*} s} e^{- \sigma y \sqrt{s} + \frac{\sigma^{2} s}{2}} d s \\ - K, 0 ] e^{- \frac{y^{2}}{2}} d y \\ \end{array}
...
$$
C_{T} = S (0) \Phi \left(y_{+}\right) - e^{- r T} K \Phi \left(y_{-}\right) \tag {25}
$$ where


$$ y_{+} := \sigma \sqrt{T} - y_{0} \quad \text{an d} \quad y_{-} := - y_{0} \tag {26}
$$ and  $y_{0}$  is the solution of the following equation (see (23))


$$
S (0) e^{- r T} e^{\sigma y_{0} \sqrt{T} - \frac{\sigma^{2} T}{2}} = K
$$ or


$$ y_{0} = \frac{\operatorname{In} \left(\frac{K}{S (0)}\right) + \left(\frac{\sigma^{2}}{2} - r\right) T}{\sigma \sqrt{T}} \tag {27}
$$ and
