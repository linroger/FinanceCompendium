...
$$
Z (t, \omega) = \int_{0}^{t} \varphi (s, \omega) d s + \int_{0}^{t} \psi (s, \omega) d B_{s} (s, \omega)
$$
...
$$
X (t, \omega) = \int_{0}^{t} \varphi (s, X) d s + \int_{0}^{t} \psi (s, X) d B_{s}
$$
...
$$ d X (t, \omega) = \varphi (t, X) d t + \psi (t, X) d B_{t}
$$
...
$$
\frac{d X (t , \omega)}{d t} = \varphi (t, X) + \psi (t, X) \frac{d B_{t}}{d t}
$$
...
$$
\Delta X (t, \omega) = \varphi^{*} (t, X) \Delta t + \psi^{*} (t, X) \Delta B_{t}
$$
...
$$
P \left[ \int_{0}^{t} b^{2} (s, \omega) d s <   \infty \text{fo ra ll} t \geq 0 \right] = 1
$$ and


$$
P \left[ \int_{0}^{t} | a (s, \omega) | d s <   \infty \text{fo ra ll} t \geq 0 \right] = 1
$$
...
$$
Z (t, \omega) = \int_{0}^{t} a (s, \omega) d s + \int_{0}^{t} b (s, \omega) d B_{s} (s, \omega)
$$

Itô processes can be written in the shorter differential form as

$$ d Z_{t} = a d t + b d B_{t}
$$
...
$$ d Y_{t} = \left(\frac{\partial g}{\partial t} (t, B_{t}) + \frac{1}{2} \frac{\partial^{2} g}{\partial x^{2}} (t, B_{t})\right) d t + \frac{\partial g}{\partial x} (t, B_{t}) d B_{t}
$$
...
$$ d Y_{t} = d t + 2 B_{t} d B_{t}
$$ or, explicitly in integral form


$$
Y_{t} = t + 2 \int_{0}^{t} B_{s} d B_{s}
$$
...
$$
\begin{array}{l} d Y_{t} = \frac{\partial g}{\partial t} (t, X_{t}) d t + \frac{\partial g}{\partial x} (t, X_{t}) d X_{t} \\ + \frac{1}{2} \frac{\partial^{2} g}{\partial x^{2}} (t, X_{t}) (d X_{t})^{2} \\ \end{array}
$$ where differentials are computed formally according to the rules known as Box algebra


$$ d t \cdot d t = d t \cdot d B_{t} = d B_{t} \cdot d t = 0, d B_{t} \cdot d B_{t} = d t
$$

Ito's formula can be written (perhaps more) explicitly as

$$ d Y_{t} = \left(\frac{\partial g}{\partial t} + \frac{\partial g}{\partial x} a + \frac{1}{2} \frac{\partial^{2} g}{\partial x^{2}} b^{2}\right) d t + \frac{\partial g}{\partial x} b d B_{t}
$$
...
$$ d Y_{t} = d \log X_{t} = \left(\mu - \frac{1}{2} \sigma^{2}\right) d t + \sigma d B_{t}
$$
...
$$
Z (t, \omega) = \int_{0}^{t} \varphi [ s, X (s, \omega) ] d s + \int_{0}^{t} \psi [ s, X (s, \omega) ] d B_{s}
$$
...
$$
X (t, \omega) = \int_{0}^{t} \varphi [ s, X (s, \omega) ] d s + \int_{0}^{t} \psi [ s, X (s, \omega) ] d B_{s}
$$
...
$$
\begin{array}{l} X (t, \omega) = X_{0} + \int_{0}^{t} \varphi [ s, X (s, \omega) ] d s \\ + \int_{0}^{t} \psi [ s, X (s, \omega) ] d B_{s} \\ \end{array}
$$
...
$$ d X (t, \omega) = \varphi [ t, X (t, \omega) ] d t + \psi [ t, X (t, \omega) ] d B_{t}
$$
...
$$
\begin{array}{l} X^{(k + 1)} (t, \omega) = \int_{0}^{t} \varphi [ s, X^{k} (s, \omega) ] d s \\ + \int_{0}^{t} \psi [ s, X^{k} (s, \omega) ] d B_{s} \\ \end{array}
$$
...
$$
\mathbf {v} = \left[ \begin{array}{l l l} v_{1 1} & \cdot & v_{1 d} \\ \cdot & \cdot & \cdot \\ v_{r 1} & \cdot & v_{r d} \end{array} \right]
...
$$
\int_{0}^{t} \mathbf {v} d B = \int_{0}^{t} \left[ \begin{array}{c c c} v_{1 1} & \cdot & v_{1 d} \\ \cdot & \cdot & \cdot \\ v_{r 1} & \cdot & v_{r d} \end{array} \right] \left[ \begin{array}{c} d B_{1} \\ \cdot \\ d B_{d} \end{array} \right]
$$ as the  $r$ -dimensional column vector whose components are the following sums of one-dimensional Itô integrals:


$$
\sum_{i = 1}^{d} \int_{0}^{t} v_{i j} (s, \omega) d B_{j} (s, \omega)
$$
...
$$ d X_{1} = u_{1} d t + v_{1 1} d B_{1} + \dots + v_{1 d} d B_{d}
$$
...
$$ d X_{1 r} = u_{r} d t + v_{r 1} d B_{1} + \dots + v_{r d} d B_{d}
...
$$
\begin{array}{l} d \mathbf {X} (t, \omega) = \mathbf {u} [ t, X_{1} (t, \omega), \dots , X_{d} (t, \omega) ] d t \\ + \mathbf {v} [ t, X_{1} (t, \omega), \dots , X_{d} (t, \omega) d B \\ \end{array}
$$
...
$$
\begin{array}{l} d Y_{k} = \frac{\partial g_{k} (t , X)}{\partial t} d t + \sum_{i} \frac{\partial g_{k} (t , X)}{\partial X_{i}} d X_{i} \\ + \frac{1}{2} \sum_{i. j} \frac{\partial^{2} g_{k} (t , X)}{\partial X_{i} \partial X_{j}} d X_{i} d X_{j} \\ \end{array}
$$
...
$$
\begin{array}{l} d X_{t} = [ A (t) X_{t} + a (t) ] d t + \sigma (t) d B_{t}, 0 \leq t <   \infty \\ X_{0} = \xi \\ \end{array}
...
$$
\begin{array}{l} d X_{t} = \mu d t + \sigma d B_{t}, \quad 0 \leq t <   \infty \\ X_{0} = \xi , \mu , \sigma \text{co ns ta nt s} \\ \end{array}
$$
...
$$ d X_{t} = [ A (t) X_{t} + a (t) ] d t + \sigma (t) d B_{t}, 0 \leq t <   \infty
$$
...
$$
\begin{array}{l} X (t) = \Phi (t) \left[ \xi + \int_{0}^{t} \Phi^{- 1} (s) a (s) d s \right. \\ \left. + \int_{0}^{t} \Phi^{- 1} (s) \sigma (s) d B_{s} \right], 0 \leq t <   \infty \\ \end{array}
$$
...
$$
\int_{0}^{t} \Phi^{- 1} (s) \sigma (s) d B_{s}
$$
...
$$
X_{t} = X_{0} + e^{- \alpha t} + \sigma \int_{0}^{t} e^{- \alpha (t - s)} d B_{s}
$$
...
$$
X_{t} = x_{0} \exp \left\{\left(\mu - \frac{1}{2} \sigma^{2}\right) t + \sigma B_{t} \right\}
$$
