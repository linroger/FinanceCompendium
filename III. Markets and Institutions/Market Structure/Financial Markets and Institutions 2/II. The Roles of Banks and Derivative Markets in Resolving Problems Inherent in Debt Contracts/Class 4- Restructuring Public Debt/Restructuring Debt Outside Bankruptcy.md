...
The future ("date 2") value of the firm is 2, 1, or 0, with probability $\mathbf{q}_2$, $\mathbf{q}_1$, and $\mathbf{q}_0$ respectively. The current going concern value of the firm is then $2\mathbf{q}_2 + \mathbf{q}_1$.

Let $q_{2} = .2$, $q_{1} = .6$ and $q_{0} = .2$. The going concern value, $\mathrm{V}_{\mathrm{GO}}$, is the $2(.2) + .6(1) = 1.0$.
