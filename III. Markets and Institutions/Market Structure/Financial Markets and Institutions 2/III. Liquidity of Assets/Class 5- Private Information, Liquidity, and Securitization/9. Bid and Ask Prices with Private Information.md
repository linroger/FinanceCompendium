---
title: Class Note 9 Bid and Ask Prices with Adverse Selection/Private Information Based on The Only Game in Town, and Glosten-Milgrom
...
If the buy is from a liquidity trader, the stock is equally likely to be worth 0 as 1, so the average profit from the trade is: ask $- [(1/2 \times 1) + (1/2 \times 0)] = \text{ask} - 1/2$.

The average profit is the average of these two "profits," so break even is when

$$
\frac{1}{2} \times (ask - 1) + \frac{1}{2} \times (ask - \frac{1}{2}) = 0 \quad or
$$
...
If the sell is from a liquidity trader, the stock is equally likely to be worth 0 as 1, so the average profit from the trade is:  $[(1/2 \times 1) + (1/2 \times 0)] - \text{bid} = 1/2 - \text{bid}$ .

A similar calculation to that for the ask, but for the bid price solves:

$$
\frac{1}{2} \times (0 - \text{bid}) + \frac{1}{2} \times (\frac{1}{2} - \text{bid}) = 0 \quad or
$$
...
liquidity_trader: Liquidity Trader {
  class: trader_type
  style.fill: "#e8f5e9"
  style.stroke: "#2e7d32"
}
...
# Bid-ask spread
bid_price -> ask_price: Spread = 0.5 {
  class: spread
}

# Trader interactions
informed_trader -> market_maker: Sells when V=0\nBuys when V=1
liquidity_trader -> market_maker: Random buy/sell

# Price formation
market_maker -> bid_price: Sets bid to break even
market_maker -> ask_price: Sets ask to break even

# Information flow
informed_trader -> true_value: Knows true value
liquidity_trader -> true_value: No information {
  style.stroke-dash: 3
  style.stroke: "#666"
}
```