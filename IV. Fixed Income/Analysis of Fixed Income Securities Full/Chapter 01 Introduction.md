...
Given that U.K. has no central bank, unlike the U.S., the Bank of England serves the role of the central bank for the U.K. As a result, LIBOR symbolizes the government rate. However, technically the Bank of England is not the central bank after all, LIBOR as a result remains a private interest rate.
...
Floating rate products use LIBORs as benchmarks, mainly because companies that issue floating rate bonds cannot borrow at the Treasury rates. Although the actual situations may change, by and large, LIBORs are between AAA and AA corporate yields.
...
Combining every industry sector and every credit rating, we are able to assign each and every firm into a "cohort". For example, 11 credit ratings (from AAA to D) and 9 industry sectors result in 99 cohorts. Within each cohort, companies should be rather homogeneous since they belong to the same industry and rating. As a result, we can compute 99 cohort yield curves.
...
$$
P = \frac{c_1}{1 + y} + \frac{c_2}{(1 + y)^2} + \dots + \frac{c_n}{(1 + y)^n} \tag{1.1}
$$

where $c_j$ is the $j$-th period cash flow. The top line is discrete discounting and the bottom line is continuous discounting. In a typical case the cash flows are fixed and the principal is redeemed at the maturity:
...
The solution is $y = 6.2753\%$ in discrete time. This yield is an expected return, or internal rate of return, and not the actual return of the bond. The actual return depends on how interest rates move and how each \$60 is invested. If rates go up in the next 10 years, the reinvestment of each \$60 will earn more than 6.2753% and the final outcome will be more. On the other hand, if rates drop, the reinvestment income of the 60 coupons will suffer lower returns and the final outcome will be less.
...
$$
\begin{array}{l}
1) P = \frac{c_1}{1 + y/m} + \frac{c_2}{(1 + y/m)^2} + \dots + \frac{c_{m \times n}}{(1 + y/m)^{m \times n}} \tag{1.2} \\
2) P = \frac{c_1}{(1 + y)^{1/m}} + \frac{c_2}{(1 + y)^{2/m}} + \dots + \frac{c_{m \times n}}{(1 + y)^n} \\
\end{array}
$$
...
For quarterly ($m = 4$) where coupon payment at each quarter is \$15 (last payment is \$1015) and 40 quarters, $y = 6.2707\%$. As we can see now that more frequent discounting leads to less yield. This is intuitive in that if an investment is paying more frequently, the reinvestment income is higher and hence you do not need a high-return investment to generate the same total cash flow (in the example it is $60 \times 10 + 1000 = 1600$).

In the extreme case where discounting is continuous:

$$
\begin{array}{l}
P = c_1 e^{-y} + c_2 e^{-2y} + \dots + c_n e^{-10y} \\
980 = 60 e^{-y} + 60 e^{-2y} + \dots + 1060 e^{-10y} \\
\end{array}
$$
...
$$
P = c_1 e^{-y \times t_1} + c_2 e^{-y \times t_2} + \dots + c_n e^{-y \times t_n}
$$
...
As we can see, how to discount has an impact on the yield. So we can conclude that yields are not unique and highly dependent upon the methods used. This demonstration hopefully can convince you that we should not use yields for any serious modeling efforts but use prices.
...
Unlike equities, not all fixed income securities are quoted in terms of price. Many markets use rate quotes and the swaption market is accustomed to volatility quotes. We should note that in the Treasury auction market, bids are submitted in terms of yields.
...
and $w(t)$ is the current swap rate, $w_K$ is the strike price.

In other words, the quoting mechanism assumes that the underlying swap rate follows a log-normal distribution.

### 1.4.4 Daycount

Daycount is a very special trading convention in the world of fixed income securities. Other markets have trading conventions (e.g., CBOE does not specify maturity date as a fixed date but the Saturday of the third Friday of the expiration month). The daycount convention specifies how many days should be in a month and in a year. There are 6 popular daycount conventions:
...
**Table 1.1: Daycount Conventions**

| Beginning | Ending | Daycount | Year Fraction |
|-----------|--------|----------|---------------|
| 1/2/2003 | 2/28/2003 | 30/360 | 0.15556 |
| 1/29/2003 | 2/28/2003 | 30/360 | 0.08056 |
| 1/30/2003 | 2/28/2003 | 30/360 | 0.07778 |
| 1/31/2003 | 2/28/2003 | 30/360 | 0.07778 |
| 1/2/2003 | 3/1/2003 | 30/360 | 0.16389 |
...
| Beginning | Ending | Daycount | Code | Year Fraction |
|-----------|--------|----------|------|---------------|
| 12/2/1995 | 3/1/1996 | 0 | 30/360 | 0.24722 |
| 12/2/1995 | 3/1/1996 | 1 | A/A | 0.2459 |
| 12/2/1995 | 3/1/1996 | 2 | A/360 | 0.25 |
| 12/2/1995 | 3/1/1996 | 3 | A/365 | 0.24658 |
...
Accrued interest is another daycount convention in fixed income. For reasons given before I was born, bonds are quoted by their "clean price" instead of the real price (called "dirty price"). Investors of bonds need to compute the price (dirty price) they have to pay by adding accrued interest to the clean price.

![Accrued Interest](https://cdn-mineru.openxlab.org.cn/result/2025-12-02/50a83d59-0129-4701-a939-9f0396f0b64f/75fe7d38834e7f44a2628e41b93811ae0ec190bba283af85cb54c864116e8a4b.jpg)
...
DM (short for discount margin) is used commonly with floaters. It represents an additional income, just like OAS. Since floaters do not have a clean yield curve, DM is added to the forward rates (which represent the expected rates). Since floaters are all at par at issuance (DM = 0), DM represents how later on floaters are transacted at discount or premium. Note that a risk-free floater should always be priced at par at coupon dates. Hence a positive DM represents that the credit quality of the floater has deteriorated since its issuance. As a result, DM is a common quoting language for a floater.
...
The major building block in fixed income securities is the yield curve, or also known as the term structure of interest rates. We shall discuss all the necessary details in Chapter 2. But in this section, we shall introduce the basic idea.
...
**Example 1: From Spot Rates to Forward Rates**

| Time | Spot Rate | PV (Discrete) | PV (Continuous) | Forward Rate (Discrete) | Forward Rate (Continuous) |
|------|-----------|---------------|-----------------|------------------------|---------------------------|
| 1 | 4.00% | 0.9615 | 0.9608 | - | - |
| 2 | 5.00% | 0.9070 | 0.9048 | 6.01% | 6.00% |
| 3 | 5.50% | 0.8516 | 0.8479 | 6.51% | 6.50% |
| 4 | 5.80% | 0.7981 | 0.7929 | 6.71% | 6.70% |

**Forward Rates by Maturity:**

| Starting Period | 1-Year Forward | 2-Year Forward | 3-Year Forward |
|-----------------|----------------|----------------|----------------|
| From Now (Discrete) | 6.01% | 6.26% | 6.41% |
| From Year 1 (Discrete) | 6.51% | 6.61% | - |
| From Year 2 (Discrete) | 6.71% | - | - |
| From Now (Continuous) | 6.00% | 6.25% | 6.40% |
| From Year 1 (Continuous) | 6.50% | 6.60% | - |
| From Year 2 (Continuous) | 6.70% | - | - |

**Figure 1.3: Spot and Forward Rates**

#### From discount factors to spot and forward rates

**Example 2: From Discount Factors to Spot and Forward Rates**

| Time | Spot Rate | PV (Discrete) | PV (Continuous) | Forward Rate (Discrete) | Forward Rate (Continuous) |
|------|-----------|---------------|-----------------|------------------------|---------------------------|
| 1 | 5.26% | 0.9500 | 0.9487 | - | - |
| 2 | 5.41% | 0.9000 | 0.8975 | 5.56% | 5.56% |
| 3 | 5.57% | 0.8500 | 0.8462 | 5.88% | 5.88% |
| 4 | 5.74% | 0.8000 | 0.7949 | 6.25% | 6.25% |

**Forward Rates by Maturity:**

| Starting Period | 1-Year Forward | 2-Year Forward | 3-Year Forward |
|-----------------|----------------|----------------|----------------|
| From Now (Discrete) | 5.56% | 5.72% | 5.90% |
| From Year 1 (Discrete) | 5.88% | 6.07% | - |
| From Year 2 (Discrete) | 6.25% | - | - |
| From Now (Continuous) | 5.56% | 5.72% | 5.90% |
| From Year 1 (Continuous) | 5.88% | 6.07% | - |
| From Year 2 (Continuous) | 6.25% | - | - |

**Figure 1.4: Spot and Forward Rates**

#### From forward rates to spot rates

**Example 3: From Forward Rates to Spot Rates**

| Time | Spot Rate | PV (Discrete) | PV (Continuous) | Forward Rate (Discrete) | Forward Rate (Continuous) |
|------|-----------|---------------|-----------------|------------------------|---------------------------|
| 1 | 4.00% | 0.9615 | 0.9608 | - | - |
| 2 | 5.00% | 0.9071 | 0.9049 | 6.00% | 5.99% |
| 3 | 5.49% | 0.8517 | 0.8480 | 6.50% | 6.49% |
| 4 | 5.79% | 0.7983 | 0.7931 | 6.70% | 6.69% |

**Forward Rates by Maturity:**

| Starting Period | 1-Year Forward | 2-Year Forward | 3-Year Forward |
|-----------------|----------------|----------------|----------------|
| From Now (Discrete) | 6.00% | 6.25% | 6.40% |
| From Year 1 (Discrete) | 6.50% | 6.60% | - |
| From Year 2 (Discrete) | 6.70% | - | - |
| From Now (Continuous) | 5.99% | 6.24% | 6.39% |
| From Year 1 (Continuous) | 6.49% | 6.59% | - |
| From Year 2 (Continuous) | 6.69% | - | - |

**Figure 1.5: Spot and Forward Rates**

## 1.5 Exercises

1. Why is a stock not a fixed income security even though its price is sensitive to rate changes?
...
**Table 1.3: Zero-Coupon Bond Prices**
...
9. Using annually discounted factors, compute the price of a 6%, 4-year coupon bond. Also compute the yield of this coupon bond.