...
Whenever working on a project, look for/ create a new markdown document called usercommands.md, in which you break down, analyze, and restate the user’s request in full detail, using your contextual understanding of the conversation to fill in the missing details of the request that the user may not have explicitly mentioned, but which was implicit in their request. Do this for each of the user’s requests.
...
# Claude macOS Development Guide
...
- Create a changelog.md file in which you detail the changes made, and keep it updated after major changes.
...
`ripgrep` (often abbreviated as `rg`) is a fast, modern alternative to the classic `grep` command-line tool for searching text in files. It’s built in Rust and optimized for speed, with sensible defaults that make it particularly useful for searching codebases. Here’s a breakdown of how to use it and how it differs from `grep`:


### **Key Differences Between `ripgrep` and `grep`**
| Feature                     | `grep` (traditional)                          | `ripgrep` (`rg`)                              |
|-----------------------------|------------------------------------------------|-----------------------------------------------|
| **Speed**                   | Fast, but slower than `rg` for large datasets. | Much faster (often 2–10x) due to parallelism, efficient regex engines, and optimized file traversal. |
| **Default Behavior**        | Recurses only with `-r`/`-R`; includes all files (including binaries, hidden files, and VCS directories like `.git`). | Recurses by default; **automatically ignores** hidden files, binary files, and files/directories listed in `.gitignore`, `.ignore`, or `.rgignore` (great for codebases). |
| **Regex Support**           | Uses POSIX ERE/BRE by default; limited Unicode support. | Uses Rust’s regex engine (PCRE-like), with full Unicode support, lookaheads, backreferences, and modern regex features. |
| **File Type Filtering**     | Requires flags like `--include="*.txt"` (cumbersome). | Built-in, intuitive file type filtering (e.g., `rg -t py "pattern"` for Python files). |
| **Color Output**            | Requires `--color=auto` for color.             | Color-coded output by default (easier to read). |
| **Installation**            | Preinstalled on nearly all Unix-like systems.  | Not preinstalled; must be installed (see below). |

### **Basic `ripgrep` Usage**
`ripgrep` is designed to be intuitive. Here are common use cases:

#### 1. **Basic Search**
Search for a pattern recursively in the current directory (default behavior):
```bash
rg "search_pattern"  # Equivalent to `grep -r "search_pattern" .` but faster and smarter
```

#### 2. **Search in Specific Files/Directories**
Target a specific file or directory:
```bash
rg "pattern" path/to/file.txt  # Search in a single file
rg "pattern" src/              # Search only in the `src/` directory
```

#### 3. **Case Insensitivity**
Use `-i` to ignore case:
```bash
rg -i "hello"  # Matches "Hello", "HELLO", "hello", etc.
```

#### 4. **Filter by File Type**
`ripgrep` has built-in support for common file types (e.g., `py` for Python, `js` for JavaScript). Use `-t` to restrict searches to a type:
```bash
rg -t py "def my_function"  # Search only in Python files
rg -t js "console.log"      # Search only in JavaScript files
```
List all supported file types with `rg --type-list`.


#### 5. **Inverse Search (Exclude Matches)**
Use `-v` to show lines **not** matching the pattern:
```bash
rg -v "error"  # Show all lines that do NOT contain "error"
```

#### 6. **Count Matches**
Use `-c` to count total matches (instead of showing the lines):
```bash
rg -c "TODO"  # Count how many times "TODO" appears in the project
```

#### 7. **Show Line Numbers**
Use `-n` to include line numbers (enabled by default in some contexts, but explicit here):
```bash
rg -n "critical"  # Show matching lines with their line numbers
```

#### 8. **Include Ignored Files**
By default, `rg` skips files in `.gitignore` or hidden directories (e.g., `.git/`). Use `-u` (or `--unrestricted`) to include them:
```bash
rg -u "secret"  # Search hidden files and files in .gitignore
```

#### 9. **Search for Whole Words**
Use `-w` to match whole words only (avoids partial matches):
```bash
rg -w "cat"  # Matches "cat" but not "category" or "scat"
```
