...
For Portfolio A, the time 1 payoff is max[0,  S_{1} - \40 :
...
$$
(\Delta \times d S \times e ^ {\delta h}) + (B \times e ^ {r h}) = C _ {d}
$$

$$
(\Delta \times u S \times e ^ {\delta h}) + (B \times e ^ {r h}) = C _ {u}
$$

This is two equations in the two unknowns  $\Delta$  and  $B$ . Solving for  $\Delta$  and  $B$  gives

$$
\Delta = e ^ {- \delta h} \frac {C _ {u} - C _ {d}}{S (u - d)} \tag {1}
$$

$$
\boxed {B = e ^ {- r h} \frac {u C _ {d} - d C _ {u}}{u - d}} \tag {2}
$$
...
$$
\Delta S + B = e ^ {- r h} \left(C _ {u} \frac {e ^ {(r - \delta) h} - d}{u - d} + C _ {d} \frac {u - e ^ {(r - \delta) h}}{u - d}\right) \tag {3}
$$
...
$$
\begin{array}{l} \Delta = \frac {\mathbb {S} 2 0 - 0}{\mathbb {S} 4 1 \times (1 . 4 6 3 4 - 0 . 7 3 1 7)} = 2 / 3 \\ B = e ^ {- 0. 0 8} \frac {1 . 4 6 3 4 \times \$ 0 - 0 . 7 3 1 7 \times \$ 2 0}{1 . 4 6 3 4 - 0 . 7 3 1 7} = - \$ 1 8. 4 6 2 \\ \end{array}
$$
...
$$
\begin{array}{l} \Delta S + B = e ^ {- 0. 0 8} \left(\$ 2 0 \times \frac {e ^ {0 . 0 8} - 0 . 7 3 1 7}{1 . 4 6 3 4 - 0 . 7 3 1 7} + \$ 0 \times \frac {1 . 4 6 3 4 - e ^ {0 . 0 8}}{1 . 4 6 3 4 - 0 . 7 3 1 7}\right) \\ = \$ 8. 8 7 1 \\ \end{array}
$$
...
$$
\underbrace {\S 9 . 0 0} _ {\text {R e c e i v e o p t i o n p r e m i u m}} - \underbrace {2 / 3 \times \S 4 1} _ {\text {C o s t o f s h a r e s}} + \underbrace {\S 1 8 . 4 6 2} _ {\text {B o r r o w i n g}} = \S 0. 1 2 9
$$
...
$$
C _ {h} = \Delta \times S _ {h} + e ^ {r h} B
$$
...
$$
\boxed {p ^ {*} = \frac {e ^ {(r - \delta) h} - d}{u - d}} \tag {5}
$$
...
$$
\boxed {C = e ^ {- r h} \left[ p ^ {*} C _ {u} + (1 - p ^ {*}) C _ {d} \right]} \tag {6}
$$
...
$$ p ^ {*} u S + (1 - p ^ {*}) d S = S e ^ {(r - \delta) h} = F _ {t, t + h} \tag {8}
$$
...
$$ r _ {t, t + h} = \ln \left(S _ {t + h} / S _ {t}\right) \tag {10}
$$
...
$$
S _ {t + h} = S _ {t} e ^ {r _ {t, t + h}} \tag {11}
$$
...
$$ r _ {t, t + n h} = \sum_ {i = 1} ^ {n} r _ {t + (i - 1) h, t + i h} \tag {12}
$$
...
$$ r _ {\text {a n n u a l}} = \sum_ {i = 1} ^ {1 2} r _ {\text {m o n t h l y}, i}
$$

The variance of the annual continuously compounded return is therefore

$$
\operatorname {V a r} \left(r _ {\text {a n n u a l}}\right) = \operatorname {V a r} \left(\sum_ {i = 1} ^ {1 2} r _ {\text {m o n t h l y}, i}\right) \tag {13}
$$
...
$$
\sigma^ {2} = 1 2 \times \sigma_ {\mathrm {m o n t h l y}} ^ {2}
$$
...
$$
\sigma_ {\mathrm {m o n t h l y}} = \frac {\sigma}{\sqrt {1 2}}
$$
...
$$
F _ {t, t + h} = S _ {t} e ^ {(r - \delta) h} \tag {16}
$$
...
$$
\begin{array}{l} u S _ {t} = F _ {t, t + h} e ^ {+ \sigma \sqrt {h}} \tag {17} \\ d S _ {t} = F _ {t, t + h} e ^ {- \sigma \sqrt {h}} \\ \end{array}
$$
...
$$ u S = \ 4 1 e ^ {(0. 0 8 - 0) \times 1 + 0. 3 \times \sqrt {1}} = \ 5 9. 9 5 4 \tag {18}
$$

$$ d S = \$ 4 1 e ^ {(0. 0 8 - 0) \times 1 - 0. 3 \times \sqrt {1}} = \$ 3 2. 9 0 3
...
\Delta = \frac {\mathbb {S} 1 9 . 9 5 4 - 0}{\mathbb {S} 4 1 \times (1 . 4 6 2 3 - 0 . 8 0 2 5)} = 0. 7 3 7 6

$$

$$

B = e ^ {- 0. 0 8} \frac {1 . 4 6 2 3 \times \$ 0 - 0 . 8 0 2 5 \times \$ 1 9 . 9 5 4}{1 . 4 6 2 3 - 0 . 8 0 2 5} = - \$ 2 2. 4 0 5
...
\Delta S + B = 0. 7 3 7 6 \times \$ 4 1 - \$ 2 2. 4 0 5 = \$ 7. 8 3 9
...
S _ {u u} = u ^ {2} \times \$ 4 1 = e ^ {2 \times (0. 0 8 + 0. 3)} \times \$ 4 1 = \$ 8 7. 6 6 9

$$

$$

S _ {u d} = S _ {d u} = u \times d \times \\ \ 41 = e ^ {(0. 0 8 + 0. 3)} \times e ^ {(0. 0 8 - 0. 3)} \times \\ \ 41 = \$ 48.114

$$

$$

S _ {d d} = d ^ {2} \times \ 41 = e ^ {2 \times (0. 0 8 - 0. 3)} \times \$ 41 = \$ 2 6. 4 0 5
...
$$ e ^ {- 0. 0 8} \left(\mathbb {S} 8. 1 1 4 \times \frac {e ^ {0 . 0 8} - 0 . 8 0 3}{1 . 4 6 2 - 0 . 8 0 3} + \mathbb {S} 0 \times \frac {1 . 4 6 2 - e ^ {0 . 0 8}}{1 . 4 6 2 - 0 . 8 0 3}\right) = \mathbb {S} 3. 1 8 7
$$

Year 0, Stock Price = \ 41 . Again using equation (3):

$$ e ^ {- 0. 0 8} \left(\mathbb {S} 2 3. 0 2 9 \times \frac {e ^ {0 . 0 8} - 0 . 8 0 3}{1 . 4 6 2 - 0 . 8 0 3} + \mathbb {S} 3. 1 8 7 \times \frac {1 . 4 6 2 - e ^ {0 . 0 8}}{1 . 4 6 2 - 0 . 8 0 3}\right) = \mathbb {S} 1 0. 7 3 7
$$
...
$$
\begin{array}{l} S _ {u} = \$ 4 1 e ^ {0. 0 8 \times 1 / 3 + 0. 3 \sqrt {1 / 3}} = \$ 5 0. 0 7 1 \\ S _ {d} = \ 4 1 e ^ {0. 0 8 \times 1 / 3 - 0. 3 \sqrt {1 / 3}} = \ 3 5. 4 1 1 \\ \end{array}
$$
...
$$
\frac {e ^ {0 . 0 8 \times 1 / 3} - 0 . 8 6 3 7}{1 . 2 2 1 2 - 0 . 8 6 3 7} = 0. 4 5 6 8
$$

The option price at the node where  S = \43.246 , for example, is then given by

$$ e ^ {- 0. 0 8 \times 1 / 3} ([ \$ 1 2. 8 1 4 \times 0. 4 5 6 8 ] + [ \$ 0 \times (1 - 0. 4 5 6 8) ]) = \$ 5. 7 0 0
...
$$
\begin{array}{l} P (S, K, t) = \\ \left. \max  \left(K - S, e ^ {- r h} \left[ P (u S, K, t + h) p ^ {*} + P (d S, K, t + h) \left(1 - p ^ {*}\right) \right]\right) \right. \tag {19} \\ \end{array}
...
$$ e ^ {- 0. 0 5 \times 1 / 3} \left[ 0. 4 5 7 \times \$ 8 7. 7 4 7 + (1 - 0. 4 5 7) \times \$ 3 2. 7 7 9 \right] = \$ 5 6. 9 4 2
...
$$ u x = x e ^ {(r - r _ {f}) h + \sigma \sqrt {h}}
$$

$$ d x = x e ^ {(r - r _ {f}) h - \sigma \sqrt {h}}
$$
...
$$
\Delta \times d x e ^ {r _ {f} h} + e ^ {r h} \times B = C _ {d}
$$

$$
\Delta \times u x e ^ {r _ {f} h} + e ^ {r h} \times B = C _ {u}
$$

The risk-neutral probability of an up move in this case is given by

$$ p ^ {*} = \frac {e ^ {(r - r _ {f}) h} - d}{u - d} \tag {20}
$$
...
$$
\Delta \times (d F - F) + e ^ {r h} \times B = C _ {d}
$$

$$
\Delta \times (u F - F) + e ^ {r h} \times B = C _ {u}
$$

Solving gives

$$
\Delta = \frac {C _ {u} - C _ {d}}{F (u - d)}
$$

$$
B = e ^ {- r h} \left(C _ {u} \frac {1 - d}{u - d} + C _ {d} \frac {u - 1}{u - d}\right)
$$
...
$$ u S _ {t} = F _ {t, t + h} e ^ {+ \sigma \sqrt {h}} \quad \text {o r} \quad u = \frac {F _ {t , t + h}}{S _ {t}} e ^ {+ \sigma \sqrt {h}} \tag {22}
$$

$$ d S _ {t} = F _ {t, t + h} e ^ {- \sigma \sqrt {h}} \quad \mathrm {o r} \quad d = \frac {F _ {t , t + h}}{S _ {t}} e ^ {- \sigma \sqrt {h}}
$$
...
$$
\begin{array}{l} p ^ {*} = \frac {F _ {t , t + h} / S _ {t} - d}{u - d} \tag {23} \\ = \frac {e ^ {(r - \delta) h} - d}{u - d} \\ \end{array}
$$ and, as before,


$$

C = e ^ {- r h} \left(p ^ {*} C _ {u} + \left(1 - p ^ {*}\right) C _ {d}\right) \tag {24}
