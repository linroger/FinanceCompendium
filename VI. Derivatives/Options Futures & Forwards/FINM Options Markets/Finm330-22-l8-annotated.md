...
**Lecture 8: Financial Mathematics 33000**  
*Roger Lee*  
*2022 November 30*
...
$$
R V a r = \frac{1}{T} \sum_{n = 0}^{N - 1} \left(\log \frac{S_{t_{n + 1}}}{S_{t_{n}}}\right)^{2}
$$

Alternatively could use simple returns, letting  $\Delta S = S_{t_{n + 1}} - S_{t_n}$  and

$$
R V a r = \frac{1}{T} \sum_{n = 0}^{N - 1} \left(\frac{\Delta S}{S_{t_{n}}}\right)^{2}
$$

Realized volatility of  $S$  is
...
$$
C = C^{BS} \left(S_{t}, t, K, T, r, \sigma\right)
$$

where  $C^{BS}$  is the Black-Scholes formula.

This exists and is unique for European options under standard conditions.

The bigger the dollar price $C$, the bigger the implied vol $\sigma_{imp}$.

This gives another way of quoting an option price. Instead of saying the option is trading at $x.xx, can say it's trading at yy% vol.
...
Implied Volatility Concepts: Volatility Surface Analysis {
  shape: rectangle
  style.fill: "#e3f2fd"
}
...
Volatility Skew: Asymmetric L-shape {
  shape: oval
  style.fill: "#fff3e0"
}
...
Black-Scholes Assumptions: Constant Volatility {
  shape: diamond
  style.fill: "#fce4ec"
}

Market Reality: Time-varying Volatility {
  shape: diamond
  style.fill: "#fce4ec"
}

Implied Volatility Concepts -> Volatility Smile: Pattern observed in
Implied Volatility Concepts -> Volatility Skew: Pattern observed in
Implied Volatility Concepts -> Volatility Surface: 3D representation

Black-Scholes Assumptions -> Market Reality: Contrasted with
Market Reality -> Volatility Smile: Explains
Market Reality -> Volatility Skew: Explains
```
