...
cssclasses: academia
title: Technical Note No. 13 - Efficient Procedure for Valuing American-Style Lookback Options
linter-yaml-title-alias: Technical Note No. 13 - Efficient Procedure for Valuing American-Style Lookback Options
---

# Technical Note No. 13<sup>1</sup>
...
A number of researchers have suggested an interesting and instructive approach to valuing lookback options.2 Consider an American-style lookback put. The initial stock price is 50, the stock price volatility is  $40\%$ , the risk-free interest rate is  $10\%$  and the life of the option is 3 months. We suppose that three steps are used to model the stock price movements.
...
$$
f _ {i j} = \max  \left(Y _ {i j} - 1, e ^ {- r \Delta t} \left[ (1 - p) f _ {i + 1, j + 1} d + p f _ {i + 1, j - 1} u \right]\right)
$$
...
$$
f _ {i j} = \max  \left(Y _ {i j} - 1, e ^ {- r \Delta t} \left[ (1 - p) f _ {i + 1, j + 1} d + p f _ {i + 1, j} u \right]\right)
$$
