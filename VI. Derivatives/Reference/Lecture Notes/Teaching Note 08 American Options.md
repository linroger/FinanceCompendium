---
title: "Teaching Note 08 American Options"
date: "2025-12-20"
primary_tags: ["american options", "options pricing", "binomial trees", "early exercise"]
secondary_tags: ["no arbitrage bounds", "american call early exercise", "american put early exercise", "dividend rules", "two step tree", "multi step trees", "dynamic replication", "american put pricing", "american option bounds", "optimal exercise time"]
---

# Teaching Note 08 American Options
...
$$
C^{A}(S, K, t, T_{2}) \geq C^{A}(S, K, t, T_{1}); \quad P^{A}(S, K, t, T_{2}) \geq P^{A}(S, K, t, T_{1})
$$
...
- If $S > K \times e^{-r(T-t)}$ and $C^A(S, K, t, T) < S - K \times e^{-r(T-t)}$, then (a) short stock $S$, (b) buy the option and (c) invest $K \times e^{-r(T-t)}$. Today we get $S - C^A - K e^{-r(T-t)} > 0$.
- At maturity $T$,
...
- Since $S - K e^{-r T} > S - K > 0$, selling the option is better as $C(S, K, 0, T) > S - K$.

### American Call Options: Early Exercise

- What if we cannot sell the option? Is it optimal to exercise early?
- No. If you exercise, you get $S - K$, which at maturity is $(S - K)e^{r T}$.
- Instead, keep the call, short the stock for $S$ and buy bonds.

* At $T$, you have

$$
S e^{r T} - S_{T} + \max(S_{T} - K, 0) = \begin{cases} S e^{r T} - K = (S - K) e^{r T} + K (e^{r T} - 1) & \text{if } S_{T} > K \\ S e^{r T} - S_{T} > S e^{r T} - K & \text{if } S_{T} < K \end{cases}
$$

* Either way, it is more than $(S - K)e^{r T}$
...
$$
D^{+} = \max \mathrm{PV}[\mathrm{dividends\ until\ maturity}]
$$

$$
D^{-} = \min \mathrm{PV}[\mathrm{dividends\ until\ maturity}]
$$

- Then
- Never exercise a call if $D^{+} < K \times (1 - e^{-r T})$
- Never exercise a put if $D^{-} > K \times (1 - e^{-r T})$
...
$$
\begin{aligned}
C(S, K, t, T) &\geq S - \mathrm{PV}(D) - K e^{-r T} \\
&\geq S - D^{+} - K e^{-r T} \\
&> S - K (1 - e^{-r T}) - K e^{-r T} \\
&= S - K
\end{aligned}
...
## $\mathbf{K - S >}$ Discounted Expected Future Payoff if Wait
...
\begin{aligned}
S_{1,u} &= 123.631 \\
p_{1,u}^{E} &= e^{-r/2} E^{*}[p_{2} | S_{1,u}] = 0
\end{aligned}
$$
...
\begin{aligned}
S_{1,d} &= 80.886 \\
p_{1,d}^{E} &= e^{-r/2} E^{*}[p_{2} | S_{1,d}] \\
&= e^{-r/2} (1 - q^{*}) p_{2,dd} = 18.119
\end{aligned}
$$
...
$$
\text{Value of Option if Wait} = e^{-r/2} E^{*}[p_{2}^{A} | S_{1,d}] = e^{-r/2} E^{*}[p_{2}^{E} | S_{1,d}] = 18.119
$$
...
$$
p_{0}^{A} = e^{-r/2} E^{*}[p_{1}^{A}] = e^{-r/2} (1 - q^{*}) p_{1,d}^{A} = 10.017
$$
...
\begin{aligned}
S_{0} &= 100 \\
p_{0}^{A} &= \max\left(K - S_{0}, e^{-r/2} E^{*}[p_{1}^{A}]\right) \\
&= e^{-r/2} q^{*} p_{1,d}^{A} = 10.017
\end{aligned}
$$
...
\begin{aligned}
S_{1,u} &= 123.631 \\
p_{1,u}^{A} &= \max\left(K - S_{1,u}, e^{-r/2} E^{*}[p_{2}^{A} | S_{1,u}]\right) \\
&= 0
\end{aligned}
$$
...
\begin{aligned}
S_{1,d} &= 80.886 \\
p_{1,d}^{A} &= \max\left(K - S_{1,d}, e^{-r/2} E^{*}[p_{2}^{A} | S_{1,d}]\right) \\
&= \max(19.114, 18.119) = 19.114
\end{aligned}
$$
...
$$
V_{i,j}^{E} = e^{-r \times h} \times E^{*}[V_{i+1}^{E} | (i,j)]
$$
...
$$
V_{i,j}^{A} = \max\{g_{i,j}, e^{-r \times h} \times E^{*}[V_{i+1}^{A} | (i,j)]\}
$$
...
$$
p_{i,j}^{A} = \max\left\{K - S_{i,j}, e^{-r \times h} \times (q^{*} \times p_{i+1,j}^{A} + (1 - q^{*}) \times p_{i+1,j+1}^{A})\right\}
$$
...
$$
p^{S\&P500}(1.05) = 0.0491 \quad \text{and} \quad p^{S\&P100}(1.05) = 0.0540
$$
...
2. Compute bonds $B_{0} = e^{-r/2}(p_{1,u}^{A} - \Delta S_{1,u}) = 54.733$
3. The replicating portfolio is $P_{0} = \Delta S_{0} + B_{0} = 10.017 = p_{0}^{A}$

- At time $i = 1$ the replicating portfolio has payoffs

$$
\text{In the Up Node } P_{1,u} = \Delta S_{1,u} + B_{0} e^{r/2} = 0
$$

$$
\text{In the Down Node } P_{1,d} = \Delta S_{1,d} + B_{0} e^{r/2} = 19.114
$$
...
- $\Longrightarrow$ The fact that the counterpart forgets to exercise the American option at node $(1,d)$ makes us earn money:

Profit from suboptimal exercise of option holder $= P_{1,d} - P_{1,d}^{\text{new}} = 19.114 - 18.119 = 0.995$
