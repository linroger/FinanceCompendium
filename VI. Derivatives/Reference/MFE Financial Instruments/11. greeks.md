...
title: "The Greeks"
date: "2025-12-20"
primary_tags: ["delta", "gamma", "rho", "vega", "theta", "implied volatility"]
secondary_tags: ["hedging", "volatility smiles", "put call parity", "option greeks", "dynamic delta hedging", "gamma neutral", "theta neutral", "stochastic volatility", "jump diffusion"]
---
...
We have from the Black-Scholes formula that the price of a call option depends on the price of the underlying asset, $S$, the strike price $K$, the time to maturity, $T$, the interest rate, $r$ and the volatility, $\sigma$.<sup>2</sup> Write this as
...
Delta $\Delta = \frac{\partial \Pi}{\partial S}$. $\Delta$ measures how the option price changes when the price of the underlying asset changes.

Theta $\Theta = -\frac{\partial \Pi}{\partial T}$. $\Theta$ measures how the option price changes as the time to maturity decreases.

Rho $\rho = \frac{\partial \Pi}{\partial r}$. $\rho$ measures how the option price changes as the interest rate changes.

Vega $\upsilon = \frac{\partial \Pi}{\partial \sigma}$. $\upsilon$ measures how the option price changes as the volatility changes.

There is also another Greek that measures how $\Delta$ changes as $S$ changes. This is known as Gamma.

Gamma $\Gamma = \frac{\partial \Delta}{\partial S} = \frac{\partial^{2} \Pi}{\partial S^{2}}$. $\Gamma$ measures the rate of change of the option's $\Delta$ as the price of the underlying changes.
...
$$
C(S, K, T, r, \sigma) = S N(d_{1}) - e^{-r T} K N(d_{2})
$$
...
$$
d_{2} = d_{1} - \sigma \sqrt{T} = \frac{\ln\left(\frac{S}{K}\right) + \left(r - \frac{1}{2} \sigma^{2}\right) T}{\sigma \sqrt{T}}
$$
...
Since the Black-Scholes formula has the form $C(S, K, T, r, \sigma) = S x - K y$ where $x = N(d_1)$ and $y = e^{-r T} N(d_2)$. This therefore implies that $\frac{\partial C}{\partial S} = N(d_1)$ and $\left(\frac{\partial C}{\partial K} = -e^{-r T} N(d_2)\right)$ and therefore that $\Delta = N(d_1)$.
...
$$
\Theta_{C} = -\frac{\partial C}{\partial T} = -\frac{S N^{\prime}(d_{1}) \sigma}{2 \sqrt{T}} - r K e^{-r T} N(d_{2}).
$$
...
$$
\Theta_{P} = -\frac{\partial C}{\partial T} = -\frac{S N^{\prime}(d_{1}) \sigma}{2 \sqrt{T}} - r K e^{-r T} N(d_{2}) + r K e^{r T}
$$
...
$$
\Gamma_{C} = \frac{N^{\prime}(d_{1})}{S \sigma \sqrt{T}}
$$
...
$$
C = \frac{1}{r} \left\{ \frac{\partial C}{\partial T} + r S \frac{\partial C}{\partial S} + \frac{1}{2} \sigma^{2} S^{2} \frac{\partial^{2} C}{\partial S^{2}} \right\}
$$

which can be re-written as

$$
C = \frac{1}{r} \left\{ -\Theta + r S \Delta + \frac{1}{2} \sigma^{2} \Gamma \right\}.
$$
...
$$
\rho_{P} = K T e^{-r T} (N(d_{2}) - 1) = -K T e^{-r T} N(-d_{2})
$$
