---
title: A Primer on Credit Default Swaps (CDS)
aliases:
...
key_concepts:
cssclasses: academia
linter-yaml-title-alias: A PRIMER ON CREDIT DEFAULT SWAPS (CDS)
---
...
21 October 2008, updated 23 March 2009, 29 December 2009 (to include pricing)
...
![](https://cdn-mineru.openxlab.org.cn/result/2025-12-02/94035ba0-08b0-45e1-b7f5-8d9c2bd0ae4c/db2328d83c2d891e27899776e6a59d88f5905287f88a75c56b30001e131649c9.jpg)  
...
When first introduced to a CDS and after reflecting on figure 1, it is natural to think that the risk or exposure of parties A and B are quite asymmetric: A has what look like fixed payments and thus what

looks like a known or fixed value, while B has a large contingent liability and thus what looks like a quite uncertain value. In fact, this is far from the truth. In the example above the risk or exposure for party B is the same as the risk of owning a five-year FRN issued by C, while the exposure for A is the same as the risk of shorting a five-year FRN. Just as the FRN may go up or down due to changes in C's credit status, so the CDS may go up or down and either party A or party B may gain (or lose).
...
![](https://cdn-mineru.openxlab.org.cn/result/2025-12-02/94035ba0-08b0-45e1-b7f5-8d9c2bd0ae4c/1675ef4ff10b5fefea520cec28d4c105a4c0a63bae07d192c77f43e28e766d2b.jpg)  
...
![](https://cdn-mineru.openxlab.org.cn/result/2025-12-02/94035ba0-08b0-45e1-b7f5-8d9c2bd0ae4c/c2ecaf35ddeef5fa9248acb29a14cf7aa76206de5f4c5366df2e477cc7169672.jpg)  
Figure 3 - CDS Payments plus Offsetting Payments = FRN - libor floater

In other words, we have just proven, rather simply and without any complex mathematics, that a CDS(sell protection) is just a combination of long an FRN and short a liber floater (worth $100):
...
$$
\mathrm {C D S} (\text {b u y p r o t e c t i o n}) \Leftrightarrow - \mathrm {F R N} + \text {l i b o r f l o a t e r} = - \mathrm {F R N} + 1 0 0
$$

This is extraordinarily useful as it tells us virtually everything we want to know about the broad how and why of a CDS:
...
![](https://cdn-mineru.openxlab.org.cn/result/2025-12-02/94035ba0-08b0-45e1-b7f5-8d9c2bd0ae4c/0280db4229d83f0dd874791fac58a7eb309f6b6166e424e9f474de67cb632516.jpg)  
Figure 2 - Time-line of CDS Payments (Sell Protection)

Under the martingale modeling approach the default is assumed to occur at some random time,  $\tau$ . This is a reduced form model in the sense that the process governing default (the random time  $\tau$ ) is assumed rather than default being modeled as a result of underlying financial or economic processes. The benefit of the approach is the substantial flexibility in the stochastic process governing default, and the simplicity of the relative pricing (risk-free or equivalent martingale) framework.(See, for example, McNeil, Frey, Embrechts 2005, section 9.3.3.)
...
$$
\mathrm {P} (\tau <   t + \mathrm {d} t \mid \tau > t) = \alpha \mathrm {d} t \quad \mathrm {P} (\tau > t \mid \tau > 0) = \exp (- \alpha t).
$$

If we assume that the risk-free rate is constant at  $r$ , then the present value of receiving the coupons  $c$  is:

PV(receive coupons  $c$  at times  $t_\mathrm{k}) = \sum_{\mathrm{k}}\exp (-rt_{\mathrm{k}})\cdot c\cdot \mathrm{P}_{\mathrm{Q}}(\tau >t_{\mathrm{k}}) = \sum_{\mathrm{k}}\exp (-rt_{\mathrm{k}})\cdot c\cdot \exp (-\alpha t_{\mathrm{k}})$ .

(This assumes that coupons occur annually, or that  $c$  includes the day-count adjustment. If not then we would have  $c \cdot df$  where  $df =$  day fraction  $=$  (days between payment) / 360 or / 365 depending on the currency and appropriate money-market convention.)

The PV of paying the loss upon default is the expectation over the random default time of the loss (net of recovery). Say the loss is 100 and the recovery is fixed at  $\delta$ . Then the loss net of recovery is  $100(1 - \delta)$  and the expected value is:

PV(loss) = - Integral[Loss amount, discounted at  $r$ , times density]

$$
= - 1 0 0 \cdot (1 - \delta) \cdot \int \alpha \cdot e ^ {- r s} \cdot e ^ {- \alpha s} d s = - 1 0 0 \cdot (1 - \delta) \cdot (\alpha / (\alpha + r)) \cdot [ 1 - \exp (- (r + \alpha) T) ]
$$
...
$= \mathrm{PV}(\text{receive coupons}) - \mathrm{PV}(\text{loss})$

$$
= \sum_ {k} \exp (- r t _ {k}) \cdot c \cdot \exp (- \alpha t _ {k}) - 1 0 0 \cdot (1 - \delta) \cdot (\alpha / (\alpha + r)) \cdot [ 1 - \exp (- (r + \alpha) T) ]
$$

$$
= \sum c \cdot d f \cdot e ^ {- t (r + \alpha)} - 1 0 0 \cdot (1 - \delta) \cdot \left(1 - e ^ {- (r + \alpha) T}\right) \frac {\alpha}{\alpha + r}
$$
...
$$
c \cdot d f \cdot \left[ e ^ {- r t} \cdot P [ \text {d e f a u l t} > t ] + c \cdot d f \cdot \int \alpha \cdot e ^ {- a u} \cdot e ^ {- r u} \cdot \frac {u - (t - b)}{b} d u \right]
$$

where  $b = 1$  / coupon frequency (e.g.  $= 0.25$  for quarterly payments)

$$
= c \cdot d f \cdot e ^ {- (r + \alpha) t} \cdot \left[ 1 + \alpha \cdot \frac {e ^ {b (r + \alpha)} - 1 - b (r + \alpha)}{b \cdot (r + \alpha) ^ {2}} \right]
$$

The first term is  $c \cdot df \cdot e^{-rt} \cdot P[default > t]$ . The second term is the accrued payment ( $c \cdot df \cdot [u - (t - b)] / b$ ) times probability of default ( $\alpha \cdot e^{-au}$ ), discounted by  $e^{-ru}$ .

Because of the equivalence between the CDS and FRN this also provides a model for valuing an FRN. In fact, this provides the obvious way to calculate the parameter  $\alpha$  from the market. We can get the risk-free rate from the government bond market or swap market, and then we can back out the value for  $\alpha$  necessary to give the correct market price of the FRN.<sup>4</sup>
...
Spreads, premiums, coupons are chosen so that instruments are at par. The differences in coupon or premium are due to differences in the way premiums or coupons are paid if default occurs between payment dates.<sup>6</sup> The first rows in the table below summarize the base case.

Now let's say the credit market moves and the probability of default rises but with no change in interest rates. Say the market moves enough so that the FRN credit spreads rise by 100bp to 400bp. The next rows in the table summarize this case. The CDS, FRN, and corporate bond all fall in value, and by essentially the same amount. The changes for the CDS with and without accrued differ by  $4.8\pmb{\xi}$ , while the CDS without accrued and the FRN differ by  $5.8\pmb{\xi}$ , out of a total change of about  $\$4.00$ . This demonstrates how one can use the FRN to understand how to value the CDS and to understand how the CDS moves as the market moves.
