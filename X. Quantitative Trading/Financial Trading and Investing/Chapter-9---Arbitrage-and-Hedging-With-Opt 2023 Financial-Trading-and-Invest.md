...
Futures contracts, which provide for the transfer of a given asset at an agreed-to price at a future date.

Options, which confer the right but not obligation to buy or sell an asset at a prespecified price on or before a given date. Options are introduced with more detail in the appendix to Chapter 7.

Swaps, which provide for the exchange of one set of cash flows for another set of cash flows.
...
Consider a stock currently selling for 100 and assume for this stock that  $u$  equals 1.2 and  $d$  equals 0.8. The stock's value in the forthcoming period will be either 120 (if outcome  $u$  is realized) or 80 (if outcome  $d$  is realized). Suppose that there exists a European call trading on this particular stock during this one-time period model with an exercise price of 90. The call expires at the end of this period when the stock value is either 120 or 80 as in Figure 9.1. Thus, if the stock were to increase to 120, the call would be worth 30 ( $c_u = 30$ ), since one could exercise the call by paying 90 for a stock which is worth 120. If the stock value were to decrease to 80, the value of the call would be zero ( $c_d = 0$ ) since no one would wish to exercise by paying 90 for shares that are worth only 80. Furthermore, suppose that the current riskless return rate is 0.10. Based on this information, we should be able to determine the value of the call.
...
![](https://cdn-mineru.openxlab.org.cn/result/2025-12-05/16e750b3-3caa-4311-89e8-b5f2577187af/2b5d6d87650d7c3744a7ee06f2e89bc7587ea86c0d17363a5cfddaba86d300ed.jpg)  
...
$c_{u} = \mathrm{MAX}[0,uS_{0} - X]$  ; value of call if stock price increases

$c_{d} = \mathrm{MAX}[0,dS_{0} - X]$ ; value of call if stock price decreases

$\alpha =$  Hedge ratio

$r_f =$  Riskless return rate

$X =$  Exercise price of the call
...
Our first step in determining the value of the call is to determine  $\alpha$ , the hedge ratio. The absolute value of the hedge ratio defines the number of shares of stock that must be sold (or short sold) for each long call position in order to maintain a riskless portfolio. This riskless portfolio will be comprised of one call option along with a short position in  $-\alpha$  shares of stock, actually, the absolute value of  $\alpha$  shares of stock, since alpha is negative. In this case, the riskless hedge portfolio made up of one call option and  $\alpha$  shares of stock will have the same value whether the stock price increases to  $uS_0$  or decreases to  $dS_0$  (see Figure 9.1).

If we were to purchase one call and sell  $-\alpha$  shares of stock, our riskless hedge condition is given as follows:

$$
c _ {u} + \alpha u S _ {0} = c _ {d} + \alpha d S _ {0} \tag {9.6}
$$

$$
3 0 + \alpha \cdot 1 2 0 = 0 + \alpha \cdot 8 0
$$
...
$$
\alpha = \frac {c _ {u} - c _ {d}}{S _ {0} (d - u)}
$$

$$
\alpha = \frac {3 0 - 0}{1 0 0 (0 . 8 - 1 . 2)} = - 0. 7 5 \tag {9.7}
$$
...
$$
c _ {u} + \alpha u S _ {0} = c _ {d} + \alpha d S _ {0} = \left(c _ {0} + \alpha S _ {0}\right) \left(1 + r _ {f}\right) \tag {9.8}
$$

Thus, our problem now is to value the call,  $c_0$ . From here, we can work equally well either with outcome  $u$  or outcome  $d$ ; since it makes no difference we can choose outcome  $d$ . Note that the time-zero option value  $c_0$  can be solved for by rearranging Equation (9.8). If Equation (9.8) does not hold, or if the current price of the option is inconsistent with Equation (9.8), a riskless arbitrage opportunity will exist. Thus, we will rewrite Equation (9.8) to solve for the zero NPV condition that eliminates positive profit arbitrage opportunities:

$$
c _ {0} + \alpha S _ {0} = \frac {c _ {d} + \alpha d S _ {0}}{\left(1 + r _ {f}\right)} \tag {9.9}
$$

It is now quite simple to solve for the call value  $c_{0}$  by rewriting Equation (9.9):

$$
c _ {0} = \frac {- (1 + r _ {f}) \alpha S _ {0} + c _ {d} + \alpha d S _ {0}}{(1 + r _ {f})} \tag {9.10}
$$

$$
c _ {0} = \frac {(1 + 0 . 1 0) \cdot 0 . 7 5 \cdot 1 0 0 + 0 - 0 . 7 5 \cdot 0 . 8 \cdot 1 0 0}{(1 + 0 . 1 0)} = 2 0. 4 5 4 5
$$
...
![](https://cdn-mineru.openxlab.org.cn/result/2025-12-05/16e750b3-3caa-4311-89e8-b5f2577187af/db8a70147cf9b1985a09583eede76aeff7a35955930e604e5fd77bf145ba6e3f.jpg)  
...
$$
p = \frac {((1 + 0 . 1) - 0 . 8)}{(1 . 2 - 0 . 8)} = 0. 7 5 \quad (1 - p) = \frac {(1 . 2 - (1 + 0 . 0 5))}{(1 . 2 - 0 . 8)} = 0. 2 5
$$

The time-zero call value in this two-period binomial framework is computed with Equation (9.16) as follows:

$$
c _ {0} = \frac {(1 4 4 - 9 0) \times 0 . 7 5 ^ {2} + 2 \times 0 . 7 5 (1 - 0 . 7 5)) \times (9 6 - 9 0) + (1 - 0 . 7 5) ^ {2} \times 0}{(1 + 0 . 1 0) ^ {2}} = 2 6. 9 6 2 8 1
$$
...
$$
\alpha_ {0} = \frac {3 0 - 0}{1 0 0 (0 . 8 - 1 . 2)} = - 0. 7 5
$$

$$
\alpha_ {u} = \frac {5 4 - 6}{1 2 0 (0 . 8 - 1 . 2)} = - 1 \quad \alpha_ {d} = \frac {6 - 0}{8 0 (0 . 8 - 1 . 2)} = - 0. 1 8 7 5
$$
...
$$
c _ {0} = \frac {\sum_ {j = 0} ^ {n} \frac {n !}{j ! (n - j) !} p ^ {j} (1 - p) ^ {n - j} \operatorname {M A X} \left[ 0 , u ^ {j} d ^ {n - j} S _ {0} - X \right]}{(1 + r _ {f}) ^ {n}} \tag {9.17}
$$

Computing Equation (9.17) might be simplified by using Equation (9.18), where  $a$  is determined by Equation (9.19)<sup>2</sup>:

$$
c _ {0} = \frac {\sum_ {j = a} ^ {n} \frac {n !}{j ! (n - j) !} p ^ {j} (1 - p) ^ {n - j} \left[ u ^ {j} d ^ {n - j} S _ {o} - X \right]}{(1 + r _ {f}) ^ {n}} \tag {9.18}
$$

$$
a = \operatorname {M A X} \left[ I N T \left(\frac {\ln \left(\frac {X}{S _ {0} d ^ {n}}\right)}{\ln \left(\frac {u}{d}\right)} + 1\right), 0 \right] \tag {9.19}
$$
...
![](https://cdn-mineru.openxlab.org.cn/result/2025-12-05/16e750b3-3caa-4311-89e8-b5f2577187af/8b58a679696fab3127b656f2a58cecb65d0b9a8e57265ae033a46577e47fb921.jpg)  
FIGURE 9.3 Three-time-period binomial model.

$0.8^{3} \times 100 = 51.2$ . The time-zero call value in this three-period binomial framework is computed with Equations (9.19) and (9.18) as follows:

$$
a = \mathrm {M A X} \left[ I N T \left(\frac {\ln \left(\frac {9 0}{1 0 0 \times 0 . 8 ^ {3}}\right)}{\ln \left(\frac {1 . 2}{0 . 8}\right)} + 1\right), 0 \right] = 2
$$

$$
\begin{array}{l} {c _ {0}} {= \frac {(3 \times 0 . 7 5 ^ {2} \times 0 . 2 5) \times (1 . 2 ^ {2} \times 0 . 8 \times 1 0 0 - 9 0) + (1 \times 0 . 7 5 ^ {3}) \times (1 . 2 ^ {3} \times 1 0 0 - 9 0)}{(1 + 0 . 1 0) ^ {3}}} \\ = \frac {(0 . 4 2 1 8 7 5 \times 2 5 . 2) + (0 . 4 2 1 8 7 5 \times 8 2 . 8)}{1 . 3 3 1} = \frac {1 0 . 6 3 1 2 5 + 3 4 . 9 3 1 2 5}{1 . 3 3 1} = 3 4. 2 3 1 7 8 \\ \end{array}
$$

Put--call parity still applies<sup>3</sup>:
...
<sup>3</sup>We will discount the exercise money with a discrete discount function since the binomial model is a discrete time model.

binomial method from the actual security returns generating process. For example, following Cox, Ross, and Rubinstein (1979), derive the following to estimate probabilities of an uptick  $p$  and downtick  $(1 - p)^{4}$

$$
p = \frac {\mathrm {e} ^ {r _ {f}} - d}{u - d} (1 - p) = \frac {u - \mathrm {e} ^ {r _ {f}}}{u - d} \tag {9.20}
$$
...
$$
u = \mathrm {e} ^ {\sigma} \quad d = \frac {1}{u} \tag {9.21}
$$

or, if  $n$  and  $T$  differ from 1:

$$
u = \mathrm {e} ^ {\left(\sigma \times \sqrt {\frac {\bar {L}}{n}}\right)} \quad d = \frac {1}{u} \tag {9.22}
$$

Suppose, for example, that for a particular Wiener process,  $\sigma = 0.30$  and  $r_f = 0.05$ . Using Equations (9.20) and (9.22), we estimate  $p$ ,  $u$ , and  $d$  for a single-time-period binomial process as follows:

$$
u = \mathrm {e} ^ {0. 3} = 1. 3 4 9 8 5 8 8
$$
...
$$
u = e ^ {0. 3 \times \sqrt {0 . 5}} = 1. 2 3 6 3 1 1 1
$$

$$
d = \frac {1}{1 . 2 3 6 3 1 1 1} = 0. 8 0 8 8 5 7 8
$$

<sup>4</sup>See Appendix 9.A for derivations of  $p$ ,  $u$ , and  $d$ . Regardless, notice the somewhat minor deviation from the probability estimates given by the Equation (9.14) set. The difference between the Equations (9.14) and (9.20) sets is that Equation (9.20) allows interest  $(r_f)$  to be continuously compounded, whereas Equation (9.14) is based on discrete compounding. This distinction is not important for our purposes here.

$$
p = \frac {\mathrm {e} ^ {r _ {f} T} - d}{u - d} = 0. 5 0 6 3 8 8 1
$$

Thus, in a risk-neutral environment, there is a 0.5064 probability that the stock price will increase to 123.63 and a 0.4936 probability that the stock price will be 80.88678. Similarly, in a risk-neutral environment, there is a 0.5064 probability that the call will be worth 13.63; therefore, its current value is  $6.73 = 0.5064 \times 13.63 \times \mathrm{e}^{-0.05 \times 0.5}$ . The call value is determined by the binomial model as follows where  $n = a = 1$ :

$$
c _ {0} = \frac {\sum_ {j = a} ^ {n} \frac {n !}{j ! (n - j) !} p ^ {j} (1 - p) ^ {n - j} \left[ u ^ {j} \times d ^ {n - j} S _ {o} - X \right]}{(1 + r _ {f}) ^ {T}}
$$

$$
c _ {0} = \frac {0 . 5 0 6 3 8 8 ^ {1} \times 0 . 4 9 3 6 1 1 9 ^ {1 - 1} \times [ 1 . 2 3 6 3 1 1 ^ {1} \times 0 . 8 0 8 8 5 7 8 ^ {1 - 1} \times 1 0 0 - 1 1 0 ]}{(1 + 0 . 0 5) ^ {5}} = 6. 7 3
$$

We can also use the binomial model to value the put with identical exercise terms on the underlying stock<sup>5</sup>:

$$
p _ {0} = \frac {\sum_ {j = 0} ^ {a - 1} \frac {n !}{j ! (n - j) !} p ^ {j} (1 - p) ^ {n - j} \left[ X - u ^ {j} d ^ {n - j} S _ {o} \right]}{(1 + r _ {f}) ^ {T}}
$$

$$
p _ {0} = \frac {0 . 5 0 6 3 8 8 ^ {0} \times 0 . 4 9 3 6 1 1 9 ^ {1} \times [ 1 1 0 - 1 . 2 3 6 3 1 ^ {0} \times 0 . 8 0 8 8 5 7 8 ^ {1} \times 1 0 0 ]}{(1 + 0 . 0 5) ^ {5}} = 1 4. 0 8
$$

Now, revise the illustration by dividing the single six-month interval into two three-month intervals; that is,  $n = 2$ . We will now use a two-period binomial model to evaluate calls and puts on this stock. First, we use Equations (9.20) and (9.22) to compute  $p$ ,  $u$ , and  $d^6$ :

$$
u = \mathrm {e} ^ {\left(0. 3 \times \sqrt {\frac {0 . 5}{2}}\right)} = 1. 1 6 1 8 3 4 2
$$

$$
d = \frac {1}{1 . 1 6 1 8 3 4 2} = 0. 8 6 0 7 0 7 9
$$

$$
p = \frac {\mathrm {e} ^ {r _ {f} T / n} - d}{u - d} = 0. 5 0 4 3 4 1 5
$$

<sup>5</sup>Recall that put-call parity, demonstrated in the Options and Hedging in a Binomial Environment section, can be used to value puts. We verify this for this example by using Equation (9.3) as follows:  $14.08 = 6.73 + 110\mathrm{e}^{-0.05 \times 0.5} - 100$ .

<sup>6</sup>When there are multiple jumps per period  $(n > 1)$ , and/or when  $T$  does not equal 1,  $p = \frac{\mathrm{e}^{r_f(T / n)} - d}{u - d}$  and  $(1 - p) = \frac{u - \mathrm{e}^{r_f(T / n)}}{u - d}$ .

Thus, there is a  $0.5043^2$  probability that the stock price will increase to 134.98, a 0.5 probability that the stock price will remain unchanged at 100, and a  $0.4957^2$  probability that the stock price will decline to 74.08. Thus, there is a 0.2543 probability that the call will be exercised, in which case, it will be worth 24.98. Therefore, the call's current value is  $6.20 = 0.2543 \times 24.98 \times \mathrm{e}^{-0.05 \times 0.5}$ . Call and put values are determined by the binomial model as follows where  $n = a = 2$ :

$$
c _ {0} = \frac {\sum_ {j = a} ^ {n} \frac {n !}{j ! (n - j) !} p ^ {j} (1 - p) ^ {n - j} \left[ u ^ {j} d ^ {n - j} S _ {o} - X \right]}{(1 + r _ {f}) ^ {T}}
$$

$$
c _ {0} = \frac {0 . 5 0 4 3 ^ {2} \times 0 . 4 9 5 7 ^ {2 - 2} \times [ 1 . 1 6 1 8 5 ^ {2} \times 0 . 8 6 0 7 ^ {2 - 2} \times 1 0 0 - 1 1 0 ]}{(1 + 0 . 0 5) ^ {0 . 5}} = 6. 2 0
$$

$$
p _ {0} = \frac {\sum_ {j = 0} ^ {a} \frac {n !}{j ! (n - j) !} p ^ {j} (1 - p) ^ {n - j} \left[ X - u ^ {j} d ^ {n - j} S _ {o} \right]}{(1 + r _ {f}) ^ {T}}
$$

$$
\begin{array}{l} p _ {0} = \frac {0 . 5 0 4 3 ^ {2 - 2} \times 0 . 4 9 5 7 ^ {2} \times [ 1 1 0 - 1 . 1 6 1 8 ^ {2 - 2} \times 0 . 8 6 0 7 ^ {2} \times 1 0 0 ] + 2 \times 0 . 5 0 4 3 \times 0 . 4 9 5 7 \times [ 1 1 0 - 1 0 0 ]}{(1 + 0 . 0 5) ^ {0 . 5}} \\ = 6. 2 0 + 1 1 0 \mathrm {e} ^ {- 0. 0 5 \times 5} - 1 0 0 = 1 3. 4 8 \\ \end{array}
$$
...
The stock price relative for a given period  $t$  is defined as  $S_{t} \div S_{t-1}$ . Thus, the log of the stock price relative is defined as  $\ln[S_{t} \div S_{t-1}]$ .

TABLE 9.1 Convergence of Binomial Model to Black-Scholes Model  

<table><tr><td>n</td><td>c0</td><td>p0_</td></tr><tr><td>1</td><td>6.73</td><td>14.02</td></tr><tr><td>2</td><td>6.20</td><td>13.48</td></tr><tr><td>3</td><td>5.47</td><td>12.72</td></tr><tr><td>4</td><td>6.04</td><td>13.30</td></tr><tr><td>5</td><td>5.18</td><td>12.44</td></tr><tr><td>6</td><td>5.91</td><td>13.17</td></tr><tr><td>7</td><td>5.43</td><td>12.68</td></tr><tr><td>8</td><td>5.81</td><td>13.06</td></tr><tr><td>9</td><td>5.57</td><td>12.82</td></tr><tr><td>10</td><td>5.73</td><td>12.98</td></tr><tr><td>50</td><td>5.63</td><td>12.89</td></tr><tr><td>100</td><td>5.59</td><td>12.85</td></tr><tr><td>∞</td><td>5.59</td><td>12.85</td></tr><tr><td>Volatility</td><td>σ = 0.30</td><td></td></tr><tr><td>Riskless rate</td><td>rf= 0.05</td><td></td></tr><tr><td>Exercise price</td><td>X = 110</td><td></td></tr><tr><td>Initial stock price</td><td>S0 = 100</td><td></td></tr><tr><td>Term to expiration</td><td>t = 0.5</td><td></td></tr></table>
...
9. Stock prices follow an Ito process; that is, they follow a continuous time-random walk in two-dimensional continuous space. This simply means that stock prices are randomly distributed and can take on any positive value at any time.

From an applications perspective, one of the more useful aspects of the Black-Scholes model is that it only requires five inputs. All of these inputs, with the exception of the variance of underlying stock returns, is normally quite easily obtained:

1. The current stock price  $(S_0)$ : Use the most recent quote.  
2. The variance of returns on the underlying stock  $(\sigma^2)$ : Several methods for estimating stock volatilities were discussed in Chapter 7.
...
5. For the risk-free return rate  $(r_f)$ , use a Treasury instrument rate with an appropriate term to maturity.
...
$$
c _ {0} = S _ {0} N \left(d _ {1}\right) - \frac {X}{\mathrm {e} ^ {r _ {f} T}} N \left(d _ {2}\right) \tag {9.23}
$$

$$
d _ {1} = \frac {\ln \left(S _ {0} / X\right) + \left(r _ {f} + 0 . 5 \sigma^ {2}\right) T}{\sigma \sqrt {T}} \tag {9.24}
$$

$$
d _ {2} = d _ {1} - \sigma \sqrt {T} \tag {9.25}
$$
...
$$
d _ {1} = \frac {\ln (7 5 / 8 0) + (0 . 1 + 0 . 5 \cdot 0 . 1 6) 0 . 5}{0 . 4 \cdot \sqrt {0 . 5}} = \frac {\ln (0 . 9 3 7 5) + 0 . 0 9}{0 . 2 8 2 8} = 0. 0 9
$$
...
$$
N (d _ {1}) = N (0. 0 9) = 0. 5 3 6 \quad N (d _ {2}) = N (- 0. 1 9 2 8) = 0. 4 2
$$

$$
c _ {0} = 7 5 \cdot 0. 5 3 6 - (8 0 \cdot 0. 9 5 1 2) \cdot 0. 4 2 = 7. 9 6
$$

Sensitivities (Greeks) for this call option are computed as follows:

$$
\frac {\partial c}{\partial S} = N (d _ {1}) = \Delta = 0. 5 3 6
$$

$$
\frac {\partial^ {2} c}{\partial S ^ {2}} = \frac {\partial \Delta}{\partial S} = \frac {\partial N (d _ {1})}{\partial S} = \frac {\mathrm {e} ^ {\left(\frac {- (d _ {1} ^ {2})}{2}\right)}}{\sqrt {2 \pi}} = \Gamma = 0. 0 1 8 7
$$

$$
\frac {\partial c}{\partial T} = - r _ {f} X \mathrm {e} ^ {- r _ {f} T} N (d _ {2}) - S \frac {\sigma}{\sqrt {T}} \frac {\mathrm {e} ^ {\left(\frac {- (d _ {1} ^ {2})}{2}\right)}}{2 \sqrt {2 \pi}} = \theta = - 1 1. 6 5 2 2 D a i l y T h e t a = - 0. 0 3 2
$$

$$
\frac {\partial c}{\partial \sigma} = S \sqrt {T} \frac {\mathrm {e} ^ {\left(\frac {- (d _ {1} ^ {2})}{2}\right)}}{\sqrt {2 \pi}} = \nu = 2 1. 0 7
$$

$$
\frac {\partial c}{\partial e ^ {r _ {f}}} = T X \mathrm {e} ^ {- r _ {f} T} N (d _ {2}) = \rho = 1 6. 1 1 5 6
$$

$$
\frac {\partial c}{\partial X} = - \mathrm {e} ^ {- r _ {f} T} N (d _ {2}) = - 0. 4 0 2 9
$$
...
$$
d e l t a _ {S} \times \# _ {S} + d e l t a _ {C} \times \# _ {C} = d e l t a _ {p o r t}
$$
...
Clearly the solution to this system is  $\#_S = -0.536$ , implying that delta neutrality requires an offsetting position in 0.536 shares of underlying stock.

Next, suppose instead that we have purchased a single share of underlying stock and that we wish to maintain both delta and gamma neutrality ( $\Delta$  and  $\Gamma$  equal zero) for our portfolio. These two criteria cannot simultaneously be accomplished with only a single option with our single share. Suppose that we can take positions in a second call option, with the same exercise terms as the first except that the exercise price equals 75 rather than 80. We calculate the delta and gamma of our second option to be 0.62483 and 0.0178, respectively. Stock deltas are 1 and stock gammas are zero. Now, we will seek to solve the following system:

$$
\begin{array}{r l r} {\left[ \begin{array}{c} d e l t a _ {p o r t} \\ g a m m a _ {p o r t} \\ \# _ {S} \end{array} \right] = \left[ \begin{array}{c c c} d e l t a _ {S} & d e l t a _ {C 1} & d e l t a _ {C 2} \\ g a m m a _ {S} & g a m m a _ {C 1} & g a m m a _ {C 2} \\ 1 & 0 & 0 \end{array} \right]} & {\left[ \begin{array}{c} \# _ {S} \\ \# _ {C 1} \\ \# _ {C 2} \end{array} \right]} \\ {\mathbf {r}} & {=} & {\mathbf {Y}} & {\#} \end{array}
$$

$$
\begin{array}{r l r} {\left[ \begin{array}{c} d e l t a _ {p o r t} \\ g a m m a _ {p o r t} \\ \# _ {S} \end{array} \right] = \left[ \begin{array}{c} 0 \\ 0 \\ 1 \end{array} \right] = \left[ \begin{array}{c c c} 1 & 0. 5 3 5 8 6 & 0. 6 2 4 8 3 \\ 0 & 0. 0 1 8 7 & 0. 0 1 7 8 \\ 1 & 0 & 0 \end{array} \right]} & {\left[ \begin{array}{c} \# _ {S} \\ \# _ {C 1} \\ \# _ {C 2} \end{array} \right]} \\ {\mathbf {r}} & {=} & {\mathbf {Y}} & {\#} \end{array}
$$
...
$$
p _ {0} = - S _ {0} N (- d _ {1}) + \frac {X}{\mathrm {e} ^ {r _ {f} T}} N (- d _ {2}) \tag {9.32}
$$
...
$$
\frac {\partial p}{\partial S} = N \left(d _ {1}\right) - 1 = \Delta_ {p} = - 0. 4 6 4
$$

$$
\frac {\partial^ {2} p}{\partial S ^ {2}} = \frac {\partial \Delta_ {p}}{\partial S} = \frac {\partial N (d _ {1}) - 1}{\partial S} = \frac {\mathrm {e} ^ {\left(\frac {- (d _ {1} ^ {2})}{2}\right)}}{\sqrt {2 \pi} S \sigma \sqrt {T}} = \Gamma_ {p} = 0. 0 1 8 7
$$

$$
\frac {\partial p}{\partial T} = r _ {f} X \mathrm {e} ^ {- r _ {f} T} N (- d _ {2}) - S \frac {\sigma}{\sqrt {T}} \frac {\mathrm {e} ^ {\left(\frac {- (d _ {1} ^ {2})}{2}\right)}}{2 \sqrt {2 \Pi}} = \theta_ {p} = - 4. 1 1 0; D a i l y T h e t a = - 0. 0 1 1
$$

$$
\frac {\partial p}{\partial \sigma} = S \sqrt {T} \frac {\mathrm {e} ^ {\left(\frac {- (d _ {1} ^ {2})}{2}\right)}}{\sqrt {2 \pi}} = \nu_ {p} = 2 1. 0 6
$$

$$
\frac {\partial p}{\partial \mathrm {e} ^ {r _ {f}}} = - T X \mathrm {e} ^ {- r _ {f} T} N (- d _ {2}) = \rho_ {p} = - 2 1. 9 3
$$
...
$$
d _ {1} = \frac {\ln \left(\frac {0 . 5 5 5 6}{0 . 5 5 5 6}\right) + \left(0 . 1 - 0 . 1 + \frac {0 . 4 ^ {2}}{2}\right) \cdot 0 . 5}{0 . 4 \sqrt {0 . 5}} = \frac {0 . 0 4}{0 . 2 8 2 8} = 0. 1 4 1 4
$$

Next we calculate  $d_2$ :

$$
d _ {2} = d _ {1} - \sigma \sqrt {T} = 0. 1 4 1 4 - 0. 2 8 2 8 = - 0. 1 4 1 4
$$

Next, find cumulative normal density functions (z-values) for  $d_{1}$  and  $d_{2}$ :

$$
N \left(d _ {1}\right) = N (0. 1 4 1 4) = 0. 5 5 6
$$

$$
N (d _ {2}) = N (- 0. 1 4 1 4) = 0. 4 4 3
$$

Finally, we value the call as follows:

$$
c _ {0} = (\$ 0. 5 5 5 6 \times 0. 5 5 6) - [ \$ 0. 5 5 5 6 \times 0. 9 5 1 2 ] \times 0. 4 4 3 = \$ 0. 0 5 9 4 3
$$

We can evaluate a European put for CHF using Equation (9.36) as follows:

$$
p _ {0} = c _ {0} + X \mathrm {e} ^ {- r (d) T} - s _ {0} \mathrm {e} ^ {- r (f) T} = 0. 0 5 9 4 3 + 0. 5 5 5 6 \mathrm {e} ^ {- 0. 1 \times 0. 5} - 0. 5 5 5 6 \mathrm {e} ^ {- 0. 1 \times 0. 5} = 0. 0 5 9 4 3
$$

Consider the next example where four call options are traded on CHF (Swiss Francs) with exercise prices  $X$  expressed in U.S. dollars. One U.S. dollar is currently worth 2 Swiss francs ( $s_0 = 2$ ). We wish to evaluate calls for each of the following European currency option series depicted in Table 9.2.
...
$$
p _ {0} = c _ {0} + X \mathrm {e} ^ {- r (d) T} - s _ {0} \mathrm {e} ^ {- r (d) T}
$$

TABLE 9.2 Currency Option Illustration  

<table><tr><td>Option 1</td><td>Option 2</td><td>Option 3</td><td>Option 4</td></tr><tr><td>T=1</td><td>T=1</td><td>T=0.5</td><td>T=2</td></tr><tr><td>r(f)=0.1</td><td>r(f)=0.1</td><td>r(f)=0.1</td><td>r(f)=0.1</td></tr><tr><td>r(d)=0.05</td><td>r(d)=0.05</td><td>r(d)=0.05</td><td>r(d)=0.05</td></tr><tr><td>s0=2</td><td>s0=2</td><td>s0=2</td><td>s0=2</td></tr><tr><td>σ=0.3</td><td>σ=0.3</td><td>σ=0.3</td><td>σ=0.3</td></tr><tr><td>X=1.8</td><td>X=2</td><td>X=2.2</td><td>X=2.2</td></tr></table>

TABLE 9.3 Currency Option Values  

<table><tr><td></td><td>Option 1</td><td>Option 2</td><td>Option 3</td><td>Option 4</td></tr><tr><td>d1</td><td>0.3345</td><td>-0.0167</td><td>-0.4611</td><td>-0.2482</td></tr><tr><td>d2</td><td>0.0345</td><td>-0.3167</td><td>-0.6732</td><td>-0.6725</td></tr><tr><td>N(d1)</td><td>0.6310</td><td>0.4934</td><td>0.3224</td><td>0.4020</td></tr><tr><td>N(d2)</td><td>0.5138</td><td>0.3757</td><td>0.2504</td><td>0.2506</td></tr><tr><td>Call</td><td>0.2622</td><td>0.1780</td><td>0.0760</td><td>0.1593</td></tr></table>
...
Call cash flows (£1,000,000 × MIN[1.75 - S1,0])

Total of option transactions:

$$
\mathbb {1}, 0 0 0, 0 0 0 \times (1. 7 5 - S _ {1}) = \mathbb {S} 1, 7 5 0, 0 0 0 - (\mathbb {1}, 0 0 0, 0 0 0 \times S _ {1})
$$

Exchange of currency  $= (£1,000,000 \times S_{1})$
...
Garman, M., & Kohlagen, S. (1983). Foreign currency option values. Journal of International Money and Finance, 2, 231-237.  
...
Jarrow, R., & Rudd, A. (1983). Option pricing. Homewood, IL: Dow Jones-Irwin.  
...
<table><tr><td>X</td><td>Aug</td><td>Sep</td><td>Oct</td><td></td></tr><tr><td>110</td><td>9.500</td><td>10.500</td><td>11.625</td><td>σ = 0.20 for Aug</td></tr><tr><td>115</td><td>4.625</td><td>7.000</td><td>8.125</td><td>σ = 0.25 for Sep</td></tr><tr><td>120</td><td>1.250</td><td>3.875</td><td>5.250</td><td>σ = 0.20 for Oct</td></tr><tr><td>125</td><td>0.250</td><td>2.125</td><td>3.125</td><td>r = 0.06</td></tr><tr><td>130</td><td>0.031</td><td>0.750</td><td>1.625</td><td>S = 120</td></tr></table>

Puts  

<table><tr><td>X</td><td>Aug</td><td>Sep</td><td>Oct</td></tr><tr><td>110</td><td>0.031</td><td>0.750</td><td>1.500</td></tr><tr><td>115</td><td>0.375</td><td>1.750</td><td>2.750</td></tr><tr><td>120</td><td>1.625</td><td>6.750</td><td>4.500</td></tr><tr><td>125</td><td>5.625</td><td>6.750</td><td>7.875</td></tr><tr><td>130</td><td>10.625</td><td>10.750</td><td>11.625</td></tr></table>
...
13. Suppose that right now, the market price of one Chinese yuan (CNY) is USD0.15; that is, one can purchase or sell CNY1.00 for USD0.15. The proportional standard deviation for CNY in terms of USD is  $20\%$ . The Chinese riskless return rate over a two-year period is projected to be  $8\%$ ; U.S. rates over the same period are  $2\%$ . What is the dollar

value of a two-year European put on a single yuan if the exercise price of the put is USD0.20?

14. Consider an extension of the Exercise 21 from Chapter 8 concerned with the management of transactions exposure. The Smedley Company sold products to a Japanese client for ¥15,000,000. Payment is due six months later. Relevant data are as follows:
...
$$
a = \operatorname {M A X} \left[ \frac {\ln \left(\frac {X}{S _ {0} d ^ {n}}\right)}{\ln \left(\frac {u}{d}\right)}, 0 \right] \tag {9.19}
$$

This result is obtained by first determining the minimum number of price increases  $j$  needed for  $S_{n}$  to exceed  $X$ :

$$
S _ {n} = u ^ {j} d ^ {n - j} S _ {0} > X \tag {9.A.1.1}
$$
...
$$
j \cdot \ln (u) + n \cdot \ln (d) - j \cdot \ln (d) + \ln (S _ {0}) > \ln X
$$

$$
j \cdot \ln \left(\frac {u}{d}\right) > \ln (X) - n \cdot \ln (d) - \ln \left(S _ {0}\right) \tag {9.A.1.2}
$$

Next divide both sides by  $\ln(u / d)$  and simplify to obtain Equation (9.19), where  $a$  is the smallest non-negative integer for  $j$  for which  $S_n > X$ . Finally, we substitute Equation (9.19) into Equation (9.7) to obtain the binomial option pricing model:
...
$$
c _ {0} = S _ {0} \left[ \sum_ {j = a} ^ {n} \frac {n !}{j ! (n - j) !} \frac {(p u) ^ {j} [ (1 - p) d ] ^ {n - j}}{(1 + r _ {f}) ^ {n}} \right] - \frac {X}{(1 + r _ {f}) ^ {n}} \left[ \sum_ {j = a} ^ {n} \frac {n !}{j ! (n - j) !} p ^ {j} (1 - p) ^ {n - j} \right] \tag {9.18a}
$$

In short-hand form:

$$
c _ {0} = S _ {0} B _ {1} [ a, n, p ^ {\prime} ] - X \left(1 + r _ {f}\right) ^ {- n} B _ {2} [ a, n, p ] \tag {9.18b}
$$
...
$$
E \left[ S _ {T} \right] = S _ {0} \mathrm {e} ^ {\mu T + \frac {1}{2} \sigma^ {2} T} \approx p u S _ {0} + (1 - p) d S _ {0} \tag {9.A.1.3}
$$

$$
\sigma_ {s} ^ {2} T = S _ {0} ^ {2} \left(\mathrm {e} ^ {\sigma^ {2} T} - 1\right) \left(\mathrm {e} ^ {2 \mu T + \sigma^ {2} T}\right) \approx \left[ p u ^ {2} S _ {0} ^ {2} + (1 - p) d ^ {2} S _ {0} ^ {2} \right] - \left[ p u S _ {0} + (1 - p) d S _ {0} \right] ^ {2} \tag {9.A.1.4}
$$

Approximation 2 approaches equality as  $T$  approaches zero. Scaling  $S_0$  to one such that we work with returns rather than actual security prices, the following can be used for returns variance of a binomial process:

$$
\sigma^ {2} = [ p u ^ {2} + (1 - p) d ^ {2} ] - [ p u + (1 - p) d ] ^ {2} \tag {9.A.1.5}
$$

We will rewrite Equation (9.A.1.5) as follows<sup>11</sup>:

$$
\sigma^ {2} = p (1 - p) (u - d) ^ {2} \tag {9.A.1.6}
$$

11The following are algebraic steps to obtain Equation (9.A.1.6) from Equation (9.A.1.5):

$$
\sigma^ {2} = \lfloor p u ^ {2} + (1 - p) d ^ {2} \rfloor - [ p u + (1 - p) d ] ^ {2} = \lfloor p u ^ {2} + d ^ {2} - p d ^ {2} \rfloor - \lfloor p ^ {2} u ^ {2} + (1 - p) ^ {2} d ^ {2} + 2 p u (1 - p) d \rfloor
$$

$$
\sigma^ {2} = \lfloor p u ^ {2} + d ^ {2} - p d ^ {2} \rfloor - \lfloor p ^ {2} u ^ {2} + d ^ {2} + p ^ {2} d ^ {2} - 2 p d ^ {2} + 2 p u d - 2 p ^ {2} u d \rfloor
$$

$$
\sigma^ {2} = \left| p u ^ {2} \right| - \left| p ^ {2} u ^ {2} + p ^ {2} d ^ {2} - p d ^ {2} + 2 p u d - 2 p ^ {2} u d \right|
$$

$$
\sigma^ {2} = p \left(u ^ {2} - p u ^ {2} - p d ^ {2} + d ^ {2} - 2 u d + 2 p u d\right) = p (1 - p) \left(u ^ {2} + d ^ {2} - 2 u d\right)
$$

Assume that the binomial process will lead to expected return for a security equaling the riskless rate:

$$
\mathrm {e} ^ {\mu T + \frac {1}{2} \sigma^ {2} T} = \mathrm {e} ^ {r _ {f}} \approx p u + (1 - p) d \tag {9.A.1.7}
$$

Solving Equation (9.A.1.7) for  $p$  enables us to estimate probabilities of an uptick  $p$  and downtick  $(1 - p)$  as:

$$
p = \frac {\mathrm {e} ^ {r _ {f}} - d}{u - d} (1 - p) = \frac {u - \mathrm {e} ^ {r _ {f}}}{u - d} \tag {9.A.1.8}
$$

If we define  $d$  as  $1 / u$  such that  $ud = 1$ , we can rewrite Equation (9.A.1.6), the variance of returns, as follows:

$$
\sigma^ {2} = p (1 - p) (u - d) ^ {2} = p (1 - p) \left(\mathrm {e} ^ {\delta} - \mathrm {e} ^ {- \delta}\right) ^ {2} \tag {9.A.1.9}
$$

Thus, we have simply substituted some constant  $e^{\delta}$  for  $u$ . Substituting  $\sigma$  for  $\delta$  will provide a good approximation for variance (improving as the number of jumps in the binomial process,  $n$  approaches infinity):

$$
\sigma^ {2} = p (1 - p) \left(\mathrm {e} ^ {\delta} - \mathrm {e} ^ {- \delta}\right) ^ {2} = p (1 - p) \left(\mathrm {e} ^ {\sigma} - \mathrm {e} ^ {- \sigma}\right) ^ {2} \tag {9.A.1.10}
$$

Thus, we can use the following to estimate  $u$  and  $d$  in the binomial approximation to the Wiener process:

$$
u = \mathrm {e} ^ {\sigma} \quad d = \frac {1}{u} \tag {9.A.1.11}
$$

or, if  $n$  and  $T$  differ from 1,

$$
u = \mathrm {e} ^ {\sigma \sqrt {\frac {T}{n}}} d = \frac {1}{u} \tag {9.A.1.12}
$$

Suppose, for example, that for a particular Wiener process,  $\sigma = 0.30$  and  $r_f = 0.05$ . Using Equations (9.A.1.7) and (9.A.1.10), we estimate  $p, u,$  and  $d$  for a single time period binomial process as follows:
...
$$
\begin{array}{l} \mathrm {e} ^ {0. 0 0 5 + \frac {1}{2} \times 0. 3 ^ {2}} = \mathrm {e} ^ {0. 0 5} \\ = 0. 5 0 9 7 4 0 9 \times 1. 3 4 9 8 5 8 8 + (1 - 0. 5 0 9 7 4 0 9) \times 0. 7 4 0 8 1 8 2 = 1. 0 5 1 2 \\ \sigma^ {2} = p (1 - p) \left(\mathrm {e} ^ {\delta} - \mathrm {e} ^ {- \delta}\right) \approx 0. 5 0 9 7 4 0 9 \times (1 - 0. 5 0 9 7 4 0 9) \left(\mathrm {e} ^ {0. 3} - \mathrm {e} ^ {- 0. 3}\right) ^ {2} = 0. 0 9 2 6 9 7 4 \\ \end{array}
$$
...
Let  $V_{h} = B$  represent the value of a perfectly hedged portfolio. We can rewrite the above equation in terms of  $VH$  as follows:

$$
V _ {h} = c + \# s S \tag {9.A.2.2}
$$

Since the hedge is riskless, its return should equal the riskless rate:

$$
d V _ {h} = r _ {f} V _ {h} d t = d c + \# s d S \tag {9.A.2.3}
$$
...
$$
V _ {h} = c - \frac {\partial c}{\partial S} S \tag {9.A.2.4}
$$

$$
r _ {f} V _ {h} d t = d c - \frac {\partial c}{\partial S} d S \tag {9.A.2.5}
$$

We rearrange the above differential equation by substituting for  $V_{h}$  and solving for  $dc$ :

$$
d c = r _ {f} \left[ c - \frac {\partial c}{\partial S} S \right] d t + \frac {\partial c}{\partial S} d S \tag {9.A.2.6}
$$

which is rewritten as follows:

$$
d c = r _ {f} c d t - r _ {f} \frac {\partial c}{\partial S} S d t + \frac {\partial c}{\partial S} d S \tag {9.A.2.7}
$$

We shall assume that the instantaneous price change for the stock follows an Itô process:

$$
d S = \mu S d t + \sigma S d z \tag {9.A.2.8}
$$

which requires us to use Ito's lemma to solve for  $dc$ . Substituting this Wiener process for  $dS$  into the preceding equation, we obtain the following:

$$
d c = r _ {f} c d t - r _ {f} \frac {\partial c}{\partial S} S d t + \frac {\partial c}{\partial S} (\mu S d t + \sigma S d z) \tag {9.A.2.9}
$$

We will rewrite Equation (9.A.2.9) separating out the term in the parentheses as follows:

$$
d c = r _ {f} c d t - r _ {f} \frac {\partial c}{\partial S} S d t + \frac {\partial c}{\partial S} \mu S d t + \frac {\partial c}{\partial S} \sigma S d z \tag {9.A.2.10}
$$

$$
d c = \mu S \frac {\partial c}{\partial S} d t + \frac {\partial c}{\partial t} d t + \frac {1}{2} S ^ {2} \sigma^ {2} \frac {\partial^ {2} c}{\partial S ^ {2}} d t + \sigma S \frac {\partial c}{\partial S} d z \tag {9.A.2.11}
$$
...
$$
r _ {f} c d t - r _ {f} \frac {\partial c}{\partial S} S d t = \frac {\partial c}{\partial t} d t + \frac {1}{2} S ^ {2} \sigma^ {2} \frac {\partial^ {2} c}{\partial S ^ {2}} d t \tag {9.A.2.12}
$$

which, after dividing by  $dt$ , is rewritten:

$$
\frac {\partial c}{\partial t} = r _ {f} c - r _ {f} \frac {\partial c}{\partial S} S _ {t} - \frac {1}{2} S ^ {2} \sigma^ {2} \frac {\partial^ {2} c}{\partial S ^ {2}} \tag {9.A.2.13}
$$

This is the Black-Scholes differential equation. Its particular solution, subject to the boundary condition  $c_{T} = \mathrm{MAX}[0,S_{T} - X]$ , is given by Equations (9.A.2.14) through (9.A.2.16):

$$
c _ {0} = S _ {0} N \left(d _ {1}\right) - \frac {X}{\mathrm {e} ^ {r _ {f} T}} N \left(d _ {2}\right) \tag {9.A.2.14}
$$

$$
d _ {1} = \frac {\ln \left(\frac {S _ {0}}{X}\right) + \left(r _ {f} + \frac {1}{2} \sigma^ {2}\right) T}{\sigma \sqrt {T}} \tag {9.A.2.15}
$$

$$
d _ {2} = d _ {1} - \sigma \sqrt {T} \tag {9.A.2.16}
$$
